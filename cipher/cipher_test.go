...
func TestSigninRoundTrip(t *testing.T) {
	enc := ProcessToken(sampleToken, SigninKey, SigninSkip, SigninEncryptLen)
	if enc == sampleToken {
		t.Fatal("sign-in encryption did not change the token")
	}
	dec := ReverseToken(enc, SigninKey, SigninSkip, SigninEncryptLen)
	if dec != sampleToken {
...
func TestReserveRoundTrip(t *testing.T) {
	enc := ProcessTokenBitmix(sampleToken, ReserveKey, ReserveSkip, ReserveEncryptLen)
	if enc == sampleToken {
		t.Fatal("reserve encryption did not change the token")
	}
	dec := ReverseTokenBitmix(enc, ReserveKey, ReserveSkip, ReserveEncryptLen)
	if dec != sampleToken {
...
func TestKeySensitivity(t *testing.T) {
	encA := ProcessToken(sampleToken, "keyAAAAAAAAAAAAAAAAA", SigninSkip, SigninEncryptLen)
	encB := ProcessToken(sampleToken, "keyBBBBBBBBBBBBBBBBB", SigninSkip, SigninEncryptLen)
	if encA == encB {
		t.Fatal("sign-in cipher output is independent of the key")
	}

	rA := ProcessTokenBitmix(sampleToken, "keyAAAAAAAAAAAAAAAAA", ReserveSkip, ReserveEncryptLen)
	rB := ProcessTokenBitmix(sampleToken, "keyBBBBBBBBBBBBBBBBB", ReserveSkip, ReserveEncryptLen)
	if rA == rB {
...
func TestJSParity(t *testing.T) {
	...
	gotSignin := ProcessToken(token, SigninKey, SigninSkip, SigninEncryptLen)
	...
	gotReserve := ProcessTokenBitmix(token, ReserveKey, ReserveSkip, ReserveEncryptLen)
