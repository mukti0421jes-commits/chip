...
function resolveBundleConfigs(text) {
    ...
    while ((idx = text.indexOf(NEEDLE, idx)) !== -1) {
        ...
        // Resolve the RAW secret expression FIRST. Eagerly substituting "local vars" is unsafe:
        // the \b-word match also hits letters INSIDE string-literal args (e.g. the `e` in "e[!*"),
        // corrupting concat-built keys so they fail to decode and a decoy sitekey object is picked
        // instead. Raw-first avoids that; substitution is only a fallback.
        const _rawExpr = secretExpr;
        let secret = R.resolveExpr(_rawExpr, objStart);
        if (!secret) {
            // fallback: inline local string vars used as bare args, e.g. r(1538,n) where const ...,n="Y$pG"
            let subbed = _rawExpr;
            try {
                const region = text.slice(Math.max(0, objStart - 6000), objStart);
        ...
        if (!secret) {
            ...
            secret = _encDecodeSecretExec(text, _rawExpr, objStart);
