...
function loadPayments(){
  fetch('/api/portal/payments').then(function(r){return r.json();}).then(function(d){
    ...
    // Payments are ALSO auto-confirmed server-side every 20s (background watcher),
    // so this button is just a manual "confirm now" — the row flips to ✓ Done on its
    // own once the invoice appears for the RID.
    ...
    // savedInvoices renders download links for invoice PDFs already saved on disk —
    // these serve straight from the server with NO API / captcha / session, so they
    // work any time, even after the login session ends.
    function savedInvoices(p){ var f=p.invoiceFiles||[]; if(!f.length) return ''; var links=f.map(function(n){ return '<a href="/api/portal/invoiceFile?entryId='+encodeURIComponent(p.id)+'&name='+encodeURIComponent(n)+'" target="_blank" style="display:inline-flex;align-items:center;gap:4px;margin:3px 6px 0 0;padding:5px 9px;background:#0d1424;border:1px solid #14532d;border-radius:5px;color:#86efac;font-size:12px;text-decoration:none">&#128190; '+n+'</a>'; }).join(''); return '<div style="margin-top:8px"><div class="muted" style="font-size:11px;margin-bottom:2px">Saved invoices (no API needed):</div>'+links+'</div>'; }
    pays.forEach(function(p){
      if(p.payStatus==='done'){html+='<div style="padding:16px;border-bottom:1px solid rgba(34,211,238,.07);text-align:left"><div style="display:flex;justify-content:space-between;align-items:center"><div><b>'+p.phone+'</b><div class="muted">'+(p.mission||'')+' • '+p.createdAt+'</div>'+ridLine(p)+'</div><span class="pill">&#10003; Done</span></div>'+invoiceBox(p)+savedInvoices(p)+'</div>';}
      else if(p.payStatus==='expired'){html+='<div style="padding:16px;border-bottom:1px solid rgba(34,211,238,.07);text-align:left"><div style="display:flex;justify-content:space-between;align-items:center"><b>'+p.phone+'</b><div class="row-actions"><span class="pill" style="background:#7f1d1d;color:#fecaca">&#9200; Expired</span> '+checkBtn(p)+'</div></div>'+ridLine(p)+'<div class="muted" style="margin-top:6px;font-size:12px">Payment link expired — re-run Full Auto for a fresh link. Already paid? Confirm above or download invoice below:</div>'+invoiceBox(p)+savedInvoices(p)+'</div>';}
      else{var purl=p.paymentUrl||''; var prev=purl.length>40?purl.substring(0,40)+'…':purl; html+='<div style="padding:16px;border-bottom:1px solid rgba(34,211,238,.07);text-align:left"><div style="display:flex;justify-content:space-between;align-items:center"><b>'+p.phone+'</b><div class="row-actions">'+lifeLabel(p)+' <button class="btn btn-sm" onclick="payCopy(this,\''+purl+'\')">&#128203; Copy</button><button class="btn btn-sm" onclick="window.open(\''+purl+'\',\'_blank\')">PayNow</button> '+checkBtn(p)+'</div></div>'+ridLine(p)+'<div class="muted" style="margin-top:6px;font-family:monospace;font-size:12px" title="'+purl+'">'+prev+'</div>'+invoiceBox(p)+savedInvoices(p)+'</div>';}
    });
