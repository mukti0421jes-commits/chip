...
// --- v1: block_mix (ChaCha-style 16-word state) ---
function genBlockMix(key, len) {
    const words = new Array(16).fill(0);
    for (let i = 0; i < key.length; i++) words[i % 16] = (words[i % 16] + key.charCodeAt(i)) | 0;
    words[12] = len;
    for (let r = 0; r < 10; r++) {
        for (let c = 0; c < 4; c++) {
            const a = c, b = c + 4, cc2 = c + 8, d = c + 12;
            words[a] = (words[a] + words[b]) | 0; words[d] ^= words[a]; words[d] = (words[d] << 16 | words[d] >>> 16) | 0;
            words[cc2] = (words[cc2] + words[d]) | 0; words[b] ^= words[cc2]; words[b] = (words[b] << 12 | words[b] >>> 20) | 0;
            words[a] = (words[a] + words[b]) | 0; words[d] ^= words[a]; words[d] = (words[d] << 8 | words[d] >>> 24) | 0;
            words[cc2] = (words[cc2] + words[d]) | 0; words[b] ^= words[cc2]; words[b] = (words[b] << 7 | words[b] >>> 25) | 0;
        }
    }
    const out = [];
    for (let i = 0; i < len; i++) out[i] = CAPTCHA_CHARSET[((words[i % 16] >>> 0) + i) % CAPTCHA_ALPHA_LEN];
    return out.join('');
}

// --- v2: bitmix (6-bit Feistel network, 8 rounds) ---
function genBitmixKeys(key) {
    const keys = [];
    let h = 0;
    for (let i = 0; i < key.length; i++) h = ((h << 5) - h + key.charCodeAt(i)) | 0;
    for (let r = 0; r < 8; r++) { h = (Math.imul(h, 1103515245) + 12345) | 0; keys.push((h >>> 0) & 0x3F); }
    return keys;
}
function cryptBitmix(token, keys, decrypt) {
    const arr = token.split('');
    for (let i = 0; i < arr.length; i++) {
        let L = CAPTCHA_CHARSET.indexOf(arr[i]);
        if (L === -1) continue;
        let R = i % 64;
        if (decrypt) { for (let r = 7; r >= 0; r--) { const t = R; R = L; L = (t ^ ((R + keys[r]) & 0x3F)) & 0x3F; } }
        else { for (let r = 0; r < 8; r++) { const t = R; R = (L ^ ((R + keys[r]) & 0x3F)) & 0x3F; L = t; } }
        arr[i] = CAPTCHA_CHARSET[L];
    }
    return arr.join('');
}

// --- v3: cellular_shift (Rule 30 elementary CA) ---
function genCellular(key, len) {
    let state = 0;
    for (let i = 0; i < key.length; i++) state = ((state << 1) | (key.charCodeAt(i) & 1)) & 0xFFFFFFFF;
    if (state === 0) state = 1;
    const out = [];
    for (let i = 0; i < len; i++) {
        let next = 0;
        for (let b = 31; b >= 0; b--) {
            const l = (state >>> ((b + 31) % 32)) & 1;
            const c = (state >>> b) & 1;
            const r = (state >>> ((b + 1) % 32)) & 1;
            next = (next << 1) | (l ^ (c | r));
        }
        state = next >>> 0;
        out[i] = CAPTCHA_CHARSET[(state >>> 16) % CAPTCHA_ALPHA_LEN];
    }
    return out.join('');
}

// --- v4: rc4_shift (RC4 KSA+PRGA over 64-element state) ---
function genRC4(key, len) {
    const S = [];
    for (let i = 0; i < CAPTCHA_ALPHA_LEN; i++) S[i] = i;
    let j = 0;
    for (let i = 0; i < 256; i++) { j = (j + S[i % CAPTCHA_ALPHA_LEN] + key.charCodeAt(i % key.length)) % CAPTCHA_ALPHA_LEN; [S[i % CAPTCHA_ALPHA_LEN], S[j]] = [S[j], S[i % CAPTCHA_ALPHA_LEN]]; }
    let ii = 0, jj = 0;
    const out = [];
    for (let k = 0; k < len; k++) {
        ii = (ii + 1) % CAPTCHA_ALPHA_LEN;
        jj = (jj + S[ii]) % CAPTCHA_ALPHA_LEN;
        [S[ii], S[jj]] = [S[jj], S[ii]];
        out[k] = S[(S[ii] + S[jj]) % CAPTCHA_ALPHA_LEN];
    }
    return out.join('');
}

// --- v5: lfsr_shift (3 LFSRs + Geffe generator) ---
function genLFSR(key, len) {
    const seeds = [74565, 424090, 773615];
    let s0 = seeds[0], s1 = seeds[1], s2 = seeds[2];
    for (let i = 0; i < key.length; i++) { s0 = (s0 ^ (key.charCodeAt(i) << 8)) >>> 0; s1 = (s1 ^ (key.charCodeAt(i) << 4)) >>> 0; s2 = (s2 ^ key.charCodeAt(i)) >>> 0; }
    const out = [];
    for (let k = 0; k < len; k++) {
        const b0 = (s0 >>> 0).toString(2).padStart(32, '0');
        const b1 = (s1 >>> 0).toString(2).padStart(32, '0');
        const b2 = (s2 >>> 0).toString(2).padStart(32, '0');
        const fb0 = parseInt(b0[0]) ^ parseInt(b0[10]) ^ parseInt(b0[30]) ^ parseInt(b0[31]);
        const fb1 = parseInt(b1[0]) ^ parseInt(b1[2]) ^ parseInt(b1[11]) ^ parseInt(b1[31]);
        const fb2 = parseInt(b2[0]) ^ parseInt(b2[5]) ^ parseInt(b2[15]) ^ parseInt(b2[31]);
        s0 = ((s0 << 1) | fb0) >>> 0; s1 = ((s1 << 1) | fb1) >>> 0; s2 = ((s2 << 1) | fb2) >>> 0;
        const g = (parseInt(b0[31]) & parseInt(b1[31])) ^ ((1 ^ parseInt(b0[31])) & parseInt(b2[31]));
        out[k] = CAPTCHA_CHARSET[g % CAPTCHA_ALPHA_LEN];
    }
    return out.join('');
}

// --- v6: polynomial (Horner evaluation over GF(67)) ---
function genPolynomial(key, len) {
    const P = 67;
    const coeffs = [];
    let h = 0;
    for (let i = 0; i < key.length; i++) h = ((h << 5) - h + key.charCodeAt(i)) | 0;
    for (let i = 0; i < 8; i++) { h = (Math.imul(h, 1103515245) + 12345) | 0; coeffs.push(((h >>> 0) % (P - 1)) + 1); }
    const out = [];
    for (let x = 1; x <= len; x++) {
        let val = 0;
        for (let c = coeffs.length - 1; c >= 0; c--) val = (val * x + coeffs[c]) % P;
        out[x - 1] = CAPTCHA_CHARSET[((val + x) | 0) % CAPTCHA_ALPHA_LEN];
    }
    return out.join('');
}
...
function buildSbox(key) {
    const box = [];
    for (let i = 0; i < CAPTCHA_ALPHA_LEN; i++) box[i] = i;
    let j = 0;
    for (let i = 0; i < CAPTCHA_ALPHA_LEN; i++) { j = (j + box[i] + key.charCodeAt(i % key.length)) % CAPTCHA_ALPHA_LEN; [box[i], box[j]] = [box[j], box[i]]; }
    return box;
}
function cryptSubstReverse(token, sbox, decrypt) {
    const arr = token.split('');
    const inv = decrypt ? new Array(CAPTCHA_ALPHA_LEN) : null;
    if (decrypt) { for (let i = 0; i < CAPTCHA_ALPHA_LEN; i++) inv[sbox[i]] = i; }
    for (let i = 0; i < arr.length; i++) {
        let idx = CAPTCHA_CHARSET.indexOf(arr[i]);
        if (idx === -1) continue;
        idx = decrypt ? inv[idx] : sbox[idx];
        arr[i] = CAPTCHA_CHARSET[idx];
    }
    return arr.reverse().join('');
}

// --- v8: prng (LCG with self-modifying multiplier) ---
function genPRNG(key, len) {
    let state = 314159265;
    let mult = 1103515245;
    for (let i = 0; i < key.length; i++) { state = (state + key.charCodeAt(i)) >>> 0; mult = (mult ^ (key.charCodeAt(i) << (i % 16))) >>> 0; }
    const out = [];
    for (let i = 0; i < len; i++) {
        state = (Math.imul(state, mult) + 12345) >>> 0;
        mult = (mult + (state & 0xFF)) >>> 0;
        out[i] = CAPTCHA_CHARSET[(state >>> 16) % CAPTCHA_ALPHA_LEN];
    }
    return out.join('');
}
...
function genModSquare(key, len) {
    const M = 0xe8d6ca6163;
    let x = 314159265;
    for (let i = 0; i < key.length; i++) x = (x + key.charCodeAt(i)) % M;
    if (x === 0) x = 314159265;
    const out = [];
    for (let i = 0; i < len; i++) {
        x = Number(BigInt(x) * BigInt(x) % BigInt(M));
        out[i] = CAPTCHA_CHARSET[x % CAPTCHA_ALPHA_LEN];
    }
    return out.join('');
}

// --- v10: logistic_shift (chaotic logistic map r=3.99) ---
function genLogistic(key, len) {
    let x = 0.5;
    for (let i = 0; i < key.length; i++) { x += key.charCodeAt(i) * 1e-9; if (x >= 1) x -= 1; }
    if (x <= 0 || x >= 1) x = 0.5;
    for (let w = 0; w < 100; w++) x = 3.99 * x * (1 - x);
    const out = [];
    for (let i = 0; i < len; i++) {
        x = 3.99 * x * (1 - x);
        out[i] = CAPTCHA_CHARSET[Math.floor(x * CAPTCHA_ALPHA_LEN) % CAPTCHA_ALPHA_LEN];
    }
    return out.join('');
}

// --- VERSION DISPATCH ---
function encryptByVersion(version, token, key, prefixLen, encodeLen) {
    const v = parseInt(version) || 1;
    const prefix = token.slice(0, prefixLen);
    const mid = token.slice(prefixLen, prefixLen + encodeLen);
    const suffix = token.slice(prefixLen + encodeLen);
    let encrypted;

    switch (v) {
        case 1: {
            const mask = genBlockMix(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
            break;
        }
        case 2: {
            const keys = genBitmixKeys(key);
            encrypted = cryptBitmix(mid, keys, false);
            break;
        }
        case 3: {
            const mask = genCellular(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
        ...
        case 4: {
            const mask = genRC4(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
        ...
        case 5: {
            const mask = genLFSR(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
        ...
        case 6: {
            const mask = genPolynomial(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
        ...
        case 7: {
            ...
            encrypted = cryptSubstReverse(mid, sbox, false);
            break;
        }
        case 8: {
            const mask = genPRNG(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
        ...
        case 9: {
            const mask = genModSquare(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
        ...
        case 10: {
            const mask = genLogistic(key, mid.length);
            encrypted = mid.split('').map((ch, i) => {
                const idx = CAPTCHA_CHARSET.indexOf(ch);
                return idx === -1 ? ch : CAPTCHA_CHARSET[(idx + CAPTCHA_CHARSET.indexOf(mask[i])) % CAPTCHA_ALPHA_LEN];
            }).join('');
...
const ENC_SIGNIN_KEY  = 'rj_enc_signin_cfg';
const ENC_RESERVE_KEY = 'rj_enc_reserve_cfg';
...
function encryptTokenByPurpose(rawToken, purpose) {
    ...
    try {
        const result = encryptByVersion(cfg.version, rawToken, cfg.key, cfg.skip, cfg.length);
