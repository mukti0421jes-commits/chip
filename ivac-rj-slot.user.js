...
(function() {
    ...
    const H2 = {
        ...
        async _silentKeepAlivePing() {
            return new Promise((resolve) => {
                ...
                gmApi({
                    ...
                    timeout: 4000,
                    onload: () => { clearTimeout(timeoutId); resolve(); },
                    onerror: () => { clearTimeout(timeoutId); resolve(); },
                    ontimeout: () => { clearTimeout(timeoutId); resolve(); }
                });
            });
        },
        ...
        async fetchH2(url, init = {}) {
            if (!this._state.warmed) { this.preWarm().catch(() => {}); }
            const h2Init = { ...init, credentials: 'omit' };
    ...
    // File endpoints — current bundle (2026-08-13) renamed these (was /file/upload-file, /file/overview):
    const API_UPLOAD_FILE   = "https://api.ivacbd.com/iams/api/v1/file/upload_file_v2232";
    const API_FILE_OVERVIEW = "https://api.ivacbd.com/iams/api/v1/file/over-view-v566";
    // x-sec-runtime-state: a security header the site now sends on upload. It is generated by the live
    // app at runtime, so we cannot hardcode it — instead we learn it by hooking the site's OWN requests
    // and replay the latest seen value. If never seen, upload proceeds without it (may be rejected).
    function getSecRuntimeState() { try { return sessionStorage.getItem('rj_sec_runtime_state') || localStorage.getItem('rj_sec_runtime_state') || ''; } catch (e) { return ''; } }
    ...
    // ==================== UNIFIED CAPTCHA TOKEN ENCRYPTION MANAGER ====================
    const ENC_UNIFIED_SCRIPT_KEY = 'rj_enc_unified_script';
    const ENC_UNIFIED_CHECK_KEY  = 'rj_enc_unified_check';

    const encManager = {
        _unifiedFn: null,

        isEnabled() {
            return !!(document.getElementById('enc-unified-check')?.checked);
        },

        encryptToken(rawToken, purpose) {
            if (!rawToken || typeof rawToken !== 'string') return rawToken;
            if (this.isEnabled() && this._unifiedFn) {
                try {
                    const result = this._unifiedFn(rawToken, purpose);
                    console.log(`[RJ Enc] ${purpose} encryption applied (Frontend)`);
                    return result;
                } catch(e) {
            ...
            console.log(`[RJ Enc] ${purpose} encryption applied (Hardcode)`);
            return encryptCaptchaTokenHardcode(rawToken);
        },

        loadUnifiedScript(scriptText) {
            if (!scriptText || !scriptText.trim()) { this._unifiedFn = null; return true; }
            try {
                ...
                fn('', '');
                this._unifiedFn = fn;
                console.log('[RJ Enc] ✅ Unified encryption script loaded');
                return true;
            } catch(e) {
                this._unifiedFn = null;
                console.error('[RJ Enc] Unified script parse error:', e);
                return false;
            }
        },

        saveUnifiedScript(scriptText) { try { localStorage.setItem(ENC_UNIFIED_SCRIPT_KEY, scriptText || ''); } catch(e) {} },
        getSavedUnifiedScript() { try { return localStorage.getItem(ENC_UNIFIED_SCRIPT_KEY) || ''; } catch(e) { return ''; } },
        saveEnabledCheck(checked) { try { localStorage.setItem(ENC_UNIFIED_CHECK_KEY, checked ? '1' : '0'); } catch(e) {} },
        getSavedEnabledCheck() { try { return localStorage.getItem(ENC_UNIFIED_CHECK_KEY) === '1'; } catch(e) { return false; } },

        updateStatusUI() {
            const statusEl = document.getElementById('enc-unified-status');
            if (statusEl) {
                if (this._unifiedFn) { statusEl.textContent = '✅ Loaded'; statusEl.style.color = '#4ade80'; }
                else {
                    const saved = this.getSavedUnifiedScript();
                    if (saved) { statusEl.textContent = '⚠ Not loaded'; statusEl.style.color = '#fcd34d'; }
                    else { statusEl.textContent = 'Hardcode'; statusEl.style.color = '#60a5fa'; }
                }
            }
        },

        init() {
            const unifiedScript = this.getSavedUnifiedScript();
            if (unifiedScript) {
                const ok = this.loadUnifiedScript(unifiedScript);
                if (!ok) console.error('[RJ Enc] Failed to load saved unified script');
            }
            setTimeout(() => {
                const unifiedCheck = document.getElementById('enc-unified-check');
                const unifiedTa = document.getElementById('enc-unified-script');
                if (unifiedCheck) { unifiedCheck.checked = this.getSavedEnabledCheck(); }
                if (unifiedTa) { unifiedTa.value = unifiedScript; }
                this.updateStatusUI();
            }, 200);
        }
    };
    ...
    const sessionState = {
        accessToken: null, userId: null, requestId: null, expiresAt: null, phone: null, loggedInAt: null, isVerified: false,
        appointmentId: null, abcDate: null, abcSlot: null, ivacCenter: null, mission: null, numberOfApplicants: null, totalAmount: null,
        fileUploadStatus: null, visaCodes: null, reservationId: null, reserveTtlSec: null, reserveStatus: null, reservedAt: null,
        bookedAt: null, otpVerifiedAt: null, otpExpiresAt: null
    };

    function gmGet(key, defaultValue) {
        try { if (typeof GM_getValue !== 'undefined') return GM_getValue(key, defaultValue); } catch(e) {}
        try { const raw = localStorage.getItem('gm_' + key); return raw ? JSON.parse(raw) : defaultValue; } catch(e) { return defaultValue; }
    }
    function gmSet(key, value) {
        try { if (typeof GM_setValue !== 'undefined') { GM_setValue(key, value); return; } } catch(e) {}
        try { localStorage.setItem('gm_' + key, JSON.stringify(value)); } catch(e) {}
    }
    function gmDelete(key) {
        try { if (typeof GM_deleteValue !== 'undefined') { GM_deleteValue(key); return; } } catch(e) {}
        try { localStorage.removeItem('gm_' + key); } catch(e) {}
    }

    function buildAuthStorage(state) {
        return { state: { token: state.accessToken || null, userId: state.userId || null, expiresAt: 899, isAuthenticated: !!state.accessToken, isVerified: !!state.isVerified, requestId: state.requestId || null, phone: state.phone || null, otpSentAt: state.loggedInAt || Date.now() } };
    }

    function persistSession() {
        try { localStorage.setItem('rj_session', JSON.stringify(sessionState)); } catch(e) {}
        try { const sf = buildAuthStorage(sessionState); localStorage.setItem('auth-storage', JSON.stringify(sf)); localStorage.setItem('rj-auth-storage-backup', JSON.stringify(sf)); } catch(e) {}
        try { gmSet(SESSION_GM_KEY, { ...sessionState }); } catch(e) {}
    }

    function saveSession(data, phone) {
        sessionState.isVerified = false; sessionState.otpVerifiedAt = null; sessionState.otpExpiresAt = null;
        ...
        sessionState.appointmentId = null; sessionState.abcDate = null; sessionState.abcSlot = null; sessionState.ivacCenter = null;
        sessionState.mission = null; sessionState.numberOfApplicants = null; sessionState.totalAmount = null; sessionState.fileUploadStatus = null;
        sessionState.visaCodes = null; sessionState.reservationId = null; sessionState.reserveTtlSec = null; sessionState.reserveStatus = null;
        sessionState.reservedAt = null; sessionState.bookedAt = null;
        sessionState.accessToken = data.accessToken; sessionState.userId = data.userId; sessionState.requestId = data.requestId;
        sessionState.expiresAt = data.expiresAt; sessionState.phone = phone; sessionState.loggedInAt = Date.now();
        persistSession();
    ...
    function markSessionVerified() {
        ...
        sessionState.isVerified = true; sessionState.otpVerifiedAt = Date.now();
        persistSession();
        if (!wasAlreadyVerified) { try { beepOtpOrVerify(); } catch(e) {} }
    }
    ...
    function restoreSession() {
        ...
        try {
            ...
            if (gm && gm.accessToken && gm.loggedInAt) {
                ...
                if (age < 15 * 60 * 1000) { Object.assign(sessionState, gm); restored = true; console.log('[RJ Session] Restored from GM storage (age', Math.round(age/1000), 's)'); }
                else { console.log('[RJ Session] GM session too old, discarding'); gmDelete(SESSION_GM_KEY); }
            }
        } catch(e) {}
        if (!restored) {
            try {
                const raw = localStorage.getItem('rj_session');
                if (raw) { const s = JSON.parse(raw); if (s.accessToken && s.loggedInAt) { const age = Date.now() - s.loggedInAt; if (age < 15 * 60 * 1000) { Object.assign(sessionState, s); restored = true; console.log('[RJ Session] Restored from rj_session localStorage'); } } }
            } catch(e) {}
        }
        if (!restored) {
            try {
                const raw = localStorage.getItem('auth-storage');
                if (raw) { const parsed = JSON.parse(raw); const s = parsed.state || parsed; if (s.token) { sessionState.accessToken = s.token; sessionState.userId = s.userId || null; sessionState.requestId = s.requestId || null; sessionState.phone = s.phone || null; sessionState.isVerified = !!s.isVerified; sessionState.loggedInAt = s.otpSentAt || Date.now(); restored = true; console.log('[RJ Session] Restored from auth-storage'); } }
            } catch(e) {}
        }
        if (restored) {
            persistSession();
            try {
                ...
                if (remainingMs > 0 && sessionState.isVerified) {
                    setTimeout(() => { try { if (typeof startTokenTimerWithExpiry === 'function') { startTokenTimerWithExpiry(sessionState.loggedInAt + 15 * 60 * 1000); } } catch(e) {} }, 500);
    ...
    function clearAllSession() {
        sessionState.accessToken = null; sessionState.userId = null; sessionState.requestId = null; sessionState.expiresAt = null;
        sessionState.phone = null; sessionState.loggedInAt = null; sessionState.isVerified = false; sessionState.appointmentId = null;
        sessionState.abcDate = null; sessionState.abcSlot = null; sessionState.ivacCenter = null; sessionState.mission = null;
        sessionState.numberOfApplicants = null; sessionState.totalAmount = null; sessionState.fileUploadStatus = null; sessionState.visaCodes = null;
        sessionState.reservationId = null; sessionState.reserveTtlSec = null; sessionState.reserveStatus = null; sessionState.reservedAt = null;
        sessionState.bookedAt = null; sessionState.otpVerifiedAt = null; sessionState.otpExpiresAt = null;
    ...
    setInterval(() => {
        ...
        if (age >= 15 * 60 * 1000) {
            ...
            try {
                clearAllSession();
                stopOtpTimer('signinOtp'); stopOtpTimer('advanceOtp');
                if (timers?.token?.intervalId) { clearInterval(timers.token.intervalId); timers.token.intervalId = null; timers.token.expiresAt = null; refreshTokenCdUI(); }
        ...
        try {
            ...
            if (!current) { needsRestore = true; }
            else { try { const parsed = JSON.parse(current); if (!parsed?.state?.token) needsRestore = true; } catch(e) { needsRestore = true; } }
            if (needsRestore) { const sf = buildAuthStorage(sessionState); localStorage.setItem('auth-storage', JSON.stringify(sf)); console.log('[RJ Session] auth-storage was cleared — restored from in-memory state'); }
        } catch(e) {}
    }, 1000);

    // ==================== SIGNUP STATE ====================
    const signupState = { mobileVerified: false, emailVerified: false, mobileRequestId: null, emailRequestId: null, stopRequested: false };
    function resetSignupState() { signupState.mobileVerified = false; signupState.emailVerified = false; signupState.mobileRequestId = null; signupState.emailRequestId = null; signupState.stopRequested = false; }
    ...
    function logStatus(msg, color = 'g') {
        ...
        ['ll', 'ls', 'lu', 'lf'].forEach(id => { const el = document.getElementById(id); if (el) el.innerHTML = html; });
    ...
    function renderCaptcha() {
        ...
        if (!container) return;
        if (_renderInFlight) { console.log('[RJ Captcha] render already in flight, skipping'); return; }
        if (Date.now() - _lastRenderAt < RENDER_COOLDOWN) { console.log('[RJ Captcha] render cooldown active, skipping'); return; }
        _renderInFlight = true;
        _lastRenderAt = Date.now();
        if (cfWidgetId !== null) { try { turnstile.remove(cfWidgetId); } catch(e) {} cfWidgetId = null; cfToken = null; container.innerHTML = ''; }
        try {
            cfWidgetId = turnstile.render(container, {
                sitekey: CF_SITEKEY, size: 'normal', theme: 'dark', appearance: 'always',
                callback: (token) => {
                    cfToken = token;
                    ...
                    setTimeout(() => {
                        ...
                        if (turnstileCount >= TOKEN_QUEUE_MAX) { console.log(`[RJ Turnstile] Queue full (${turnstileCount}/${TOKEN_QUEUE_MAX}) — pausing auto-solve`); return; }
                        try { turnstile.reset(cfWidgetId); cfToken = null; console.log(`[RJ Turnstile] Auto-reset for next solve (queue ${turnstileCount}/${TOKEN_QUEUE_MAX})`); }
                        catch(e) { console.log('[RJ Turnstile] reset failed:', e.message); }
                    }, 2000);
                },
                'error-callback': () => { logStatus('✗ Captcha error', 'r'); },
                'expired-callback': () => {
                    cfToken = null; logStatus('⚠ Captcha expired', 'y');
                    const isApiMode = document.getElementById('captcha-toggle')?.classList.contains('on');
                    if (!isApiMode && cfWidgetId !== null) { try { turnstile.reset(cfWidgetId); } catch(e) {} }
                },
                'timeout-callback': () => { logStatus('⚠ Captcha timeout', 'y'); }
            });
        } finally { setTimeout(() => { _renderInFlight = false; }, 2000); }
    }

    function resetCaptcha() {
        if (typeof turnstile !== 'undefined' && cfWidgetId !== null) { try { turnstile.reset(cfWidgetId); } catch(e) {} cfToken = null; }
    }

    // Manual captcha box stays as a blank placeholder (layout unchanged).
    // Widget only renders when captcha-toggle is OFF and a Signin/Reserve call needs a token.
    function showManualCaptcha() {
        const useApi = document.getElementById('captcha-toggle')?.classList.contains('on');
        if (useApi) return; // API mode → leave the box blank
        _lastRenderAt = 0; // bypass cooldown for on-demand render
    ...
    function hideManualCaptcha() {
        // keep the box visible but empty (do NOT touch display → look stays the same)
        try {
    ...
    const cfCheck = setInterval(() => {
        if (typeof turnstile !== 'undefined') {
            ...
            // NOTE: do NOT auto-render here — widget stays blank until showManualCaptcha()
            // is called by a manual-mode Signin/Reserve token request.
        }
    }, 3000);
    ...
    async function performSignin(phone, password, captchaToken) {
        ...
        const response = await H2.fetchH2(API_SIGNIN_V2, {
            method: "POST",
            headers: { "accept": "application/json, text/plain, */*", "accept-language": "en-US,en;q=0.9", "cache-control": "no-cache", "content-type": "application/json", "pragma": "no-cache", "priority": "u=1, i", "x-device-id": getDeviceId() },
    ...
    async function performInitiate(accessToken, appointmentId) {
        ...
        else { const text = await response.text(); try { result = JSON.parse(text); } catch(e) { result = { raw: text }; } }
        return { ok: response.ok, status: response.status, body: result };
    }
    ...
    function extractPaymentUrl(body) {
        ...
        return d.webview_url || d.GatewayPageURL || d.gatewayPageURL || d.paymentUrl || d.payment_url || d.gatewayUrl || d.redirectUrl || d.redirect_url || d.url || d.epayUrl || d.securePayUrl || null;
    }
    ...
    s.textContent = `
/* ===== MAIN PANEL ===== */
#p{position:fixed;top:50%;right:20px;transform:translateY(-50%);width:285px;height:auto;background:linear-gradient(160deg,#0d0d1e 0%,#10102a 60%,#0a0a18 100%);border-radius:14px;box-shadow:0 20px 50px rgba(0,0,0,.85),0 0 0 1px rgba(124,58,237,.25),0 0 25px rgba(124,58,237,.12);z-index:999999;font-family:'Segoe UI',system-ui,-apple-system,sans-serif;color:#e0e0f0;overflow:hidden;display:flex;flex-direction:column;backdrop-filter:blur(8px)}
#p.hidden{display:none !important}
#p *{box-sizing:border-box;margin:0;padding:0}
#p .dh{background:linear-gradient(90deg,#0a0a18,#15152e,#0a0a18);padding:12px 14px;cursor:move;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(124,58,237,.25);user-select:none;flex-shrink:0;position:relative}
#p .dh::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:linear-gradient(180deg,#7c3aed,#4f46e5);border-radius:0 2px 2px 0}
#p .dt{font-size:.72rem;font-weight:800;background:linear-gradient(90deg,#c4b5fd,#a78bfa,#818cf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:1px}
#p .db{background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.35);color:#a78bfa;font-size:.95rem;cursor:pointer;line-height:1;padding:0 7px;border-radius:4px;transition:all .2s}
#p .db:hover{background:rgba(124,58,237,.25);color:#fff;transform:scale(1.05)}
#p .tb{display:flex;background:#06061a;padding:0;border-bottom:1px solid rgba(124,58,237,.2);flex-shrink:0}
#p .t{background:none;border:none;padding:11px 0;color:#5a5a80;font-weight:700;font-size:.68rem;cursor:pointer;border-bottom:2px solid transparent;flex:1;text-align:center;transition:all .2s;position:relative}
#p .t:hover{color:#a78bfa;background:rgba(124,58,237,.08)}
#p .t.a{color:#fff;background:linear-gradient(180deg,rgba(124,58,237,.15),transparent);border-bottom-color:#7c3aed;text-shadow:0 0 8px rgba(124,58,237,.6)}
#p .bd{overflow:visible;flex:1}
#p .sl{background:linear-gradient(135deg,#06061a,#0a0a1f);margin:5px 8px;border-radius:6px;border:1px solid rgba(124,58,237,.2);overflow:hidden;flex-shrink:0;height:auto;min-height:34px;max-height:34px;box-shadow:inset 0 1px 3px rgba(0,0,0,.5)}
#p .sl>div{display:none;padding:4px 7px;font-family:Consolas,monospace;font-size:.68rem;color:#5a5a88;line-height:1.4;height:auto;overflow:hidden;white-space:nowrap;text-overflow:ellipsis}
#p .sl>div.a{display:block}
#p .sl .g{color:#4ade80;text-shadow:0 0 6px rgba(74,222,128,.4)}
#p .sl .r{color:#fca5a5;text-shadow:0 0 6px rgba(252,165,165,.4)}
#p .sl .y{color:#fcd34d;text-shadow:0 0 6px rgba(252,211,77,.4)}
#p .sc{padding:5px 9px 6px}
#p .fr{display:flex;gap:3px;margin-bottom:1px;align-items:center}
#p .sa-row{gap:3px}
#p .sa-row input[type=number]{flex:1;width:0;min-width:0;padding:3px 0;text-align:center;font-size:.62rem;font-weight:700}
#p .sa-btn{flex:0 0 38px;padding:3px 0!important;font-size:.6rem!important;font-weight:800;letter-spacing:.2px;border-radius:4px!important}
#p .fr-otp{display:flex;gap:4px;margin-bottom:3px;align-items:center}
#p .fr-otp input[type=text]{flex:50;min-width:0}
#p .fr-otp button.b4{flex:18;min-width:0}
#p .fr-otp button.b5{flex:32;min-width:0}
#p .fr-phone{display:flex;gap:4px;margin-bottom:3px;align-items:center}
#p .phone-type-sel{flex:none;width:58px;background:linear-gradient(180deg,#13132e,#0a0a18);border:1px solid #3b3777;color:#c4b5fd;padding:4px 2px;border-radius:4px;font-size:.6rem;font-weight:700;outline:none;cursor:pointer;text-align:center;appearance:none;box-shadow:0 1px 3px rgba(0,0,0,.4)}
#p .phone-type-sel:focus{border-color:#7c3aed;box-shadow:0 0 0 2px rgba(124,58,237,.2)}
#p .phone-type-sel option{background:#0e0e1c;color:#c4b5fd;font-size:.6rem}
#p .fr-phone input[type=text],#p .fr-phone input[type=tel],#p .fr-phone input[type=email]{flex:1;background:linear-gradient(180deg,#0a0a1a,#08081a);border:1px solid #2a2a4a;padding:5px 7px;border-radius:4px;color:#e0e0f0;font-size:.68rem;font-weight:600;outline:none;min-width:0;transition:all .2s}
#p .fr-phone input::placeholder{color:#4a4a6a;font-size:.6rem;font-weight:500}
#p .fr-phone input:focus{border-color:#7c3aed;box-shadow:0 0 0 2px rgba(124,58,237,.15)}
#p .fr-phone .sw-c{flex:0 0 24px;display:flex;align-items:center;justify-content:center}
#p .fr-pw{display:flex;gap:4px;margin-bottom:3px;align-items:center}
#p .fr-pw .pw{flex:0 0 55%;display:flex;position:relative;min-width:0}
#p .fr-pw .pw input{width:100%;background:linear-gradient(180deg,#0a0a1a,#08081a);border:1px solid #2a2a4a;padding:5px 7px;border-radius:5px;color:#e0e0f0;font-size:.68rem;font-weight:600;outline:none;padding-right:22px;transition:all .2s}
#p .fr-pw .pw input::placeholder{color:#4a4a6a;font-size:.6rem;font-weight:500}
#p .fr-pw .pw input:focus{border-color:#7c3aed;box-shadow:0 0 0 2px rgba(124,58,237,.15)}
#p .fr-pw .pw .eye{position:absolute;right:2px;top:50%;transform:translateY(-50%);background:none;border:none;color:#666690;cursor:pointer;padding:0 4px;font-size:.7rem;line-height:1;z-index:2;transition:color .2s}
#p .fr-pw .pw .eye:hover{color:#a78bfa}
#p .fr-pw .b7.bh{flex:1;padding:5px 0;font-size:.66rem;font-weight:700;min-width:0}
#p .tr{display:flex;gap:5px;margin-bottom:3px;align-items:center}
#p .tm{flex:1;border-radius:5px;padding:1px 3px;text-align:center;position:relative;overflow:hidden;height:26px;display:flex;flex-direction:column;justify-content:center;background:linear-gradient(135deg,#0a0a1a,#0d0d24);border:1px solid #1f1f3a;box-shadow:inset 0 1px 2px rgba(0,0,0,.5)}
#p .tm.t1{border-color:rgba(74,222,128,.3)}
#p .tm.t2{border-color:rgba(167,139,250,.3)}
#p .tm input.ti{width:100%;background:transparent;border:1px solid transparent;border-radius:4px;padding:1px 2px;font-family:Consolas,'Courier New',monospace;font-size:.7rem;font-weight:800;letter-spacing:.3px;text-align:center;outline:none}
#p .tm.t1 input.ti{color:#4ade80;text-shadow:0 0 10px rgba(74,222,128,.5)}
#p .tm.t2 input.ti{color:#c4b5fd;text-shadow:0 0 10px rgba(167,139,250,.5)}
#p .tm input.ti:hover{border-color:rgba(255,255,255,.12);background:rgba(255,255,255,.02)}
#p .tm input.ti:focus{border-color:rgba(255,255,255,.25);background:rgba(0,0,0,.3)}
#p .tr .reserve-tg-wrap{flex:0 0 22px;display:flex;align-items:center;justify-content:center;height:26px}
#p input[type=text],#p input[type=tel],#p input[type=email],#p input[type=password],#p input[type=number],#p select{background:linear-gradient(180deg,#0a0a1a,#08081a);border:1px solid #2a2a4a;padding:5px 7px;border-radius:4px;color:#e0e0f0;font-size:.68rem;font-weight:600;flex:1;outline:none;font-family:inherit;min-width:0;transition:all .2s}
#p input:focus,#p select:focus{border-color:#7c3aed;box-shadow:0 0 0 2px rgba(124,58,237,.15)}
#p input::placeholder{color:#4a4a6a;font-size:.6rem;font-weight:500}
#p select option{background:#0e0e1c;color:#c4b5fd}
#p input[type=number]{text-align:center;padding:4px 2px}
#p input[type=file]{font-size:.56rem;padding:3px;background:#0a0a1a;border:1px solid #1c1c36;border-radius:4px;color:#b0b0cc}
#p textarea{background:linear-gradient(180deg,#0a0a1a,#08081a);border:1px solid #1c1c36;border-radius:4px;color:#d0d0e6;font-size:.64rem;font-weight:600;padding:5px 7px;width:100%;resize:none;font-family:Consolas,monospace;outline:none;transition:all .2s}
#p textarea:focus{border-color:#7c3aed;box-shadow:0 0 0 2px rgba(124,58,237,.15)}
#p textarea::placeholder{color:#2a2a50}
#p label{font-size:.62rem;color:#7777aa;font-weight:700;white-space:nowrap}
#p button{border:none;padding:6px 9px;border-radius:5px;color:#fff;font-weight:700;font-size:.64rem;transition:all .2s;white-space:nowrap;font-family:inherit;cursor:pointer;letter-spacing:.3px;position:relative;overflow:hidden}
#p button:hover{transform:translateY(-1px);filter:brightness(1.15)}
#p button:active{transform:scale(.96)}
#p .b1{background:linear-gradient(135deg,#3b82f6,#1d4ed8);border:1px solid #60a5fa;color:#fff;box-shadow:0 2px 6px rgba(59,130,246,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b2{background:linear-gradient(135deg,#f59e0b,#d97706);border:1px solid #fbbf24;color:#fff;box-shadow:0 2px 6px rgba(245,158,11,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b3{background:linear-gradient(135deg,#14b8a6,#0d9488);border:1px solid #2dd4bf;color:#fff;box-shadow:0 2px 6px rgba(20,184,166,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b4{background:linear-gradient(135deg,#d946ef,#a21caf);border:1px solid #e879f9;color:#fff;box-shadow:0 2px 6px rgba(217,70,239,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b5{background:linear-gradient(135deg,#10b981,#059669);border:1px solid #34d399;color:#fff;box-shadow:0 2px 6px rgba(16,185,129,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b6{background:linear-gradient(135deg,#8b5cf6,#6d28d9);border:1px solid #a78bfa;color:#fff;box-shadow:0 2px 6px rgba(139,92,246,.4),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b7{background:linear-gradient(135deg,#06b6d4,#0891b2);border:1px solid #22d3ee;color:#fff;box-shadow:0 2px 6px rgba(6,182,212,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b8{background:linear-gradient(135deg,#ef4444,#b91c1c);border:1px solid #f87171;color:#fff;box-shadow:0 2px 6px rgba(239,68,68,.4),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b9{background:linear-gradient(135deg,#f43f5e,#be123c);border:1px solid #fb7185;color:#fff;box-shadow:0 2px 6px rgba(244,63,94,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b10{background:linear-gradient(135deg,#0ea5e9,#0369a1);border:1px solid #38bdf8;color:#fff;box-shadow:0 2px 6px rgba(14,165,233,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b11{background:linear-gradient(135deg,#84cc16,#4d7c0f);border:1px solid #a3e635;color:#fff;box-shadow:0 2px 6px rgba(132,204,22,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b12{background:linear-gradient(135deg,#a855f7,#7e22ce);border:1px solid #c084fc;color:#fff;box-shadow:0 2px 6px rgba(168,85,247,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b13{background:linear-gradient(135deg,#dc2626,#7f1d1d);border:1px solid #ef4444;color:#fff;box-shadow:0 2px 6px rgba(220,38,38,.35),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b14{background:linear-gradient(135deg,#6366f1,#3730a3);border:1px solid #818cf8;color:#fff;box-shadow:0 2px 6px rgba(99,102,241,.4),inset 0 1px 0 rgba(255,255,255,.15)}
#p .b15{background:linear-gradient(135deg,#ea580c,#f59e0b);border:1px solid #fb923c;color:#fff;box-shadow:0 2px 8px rgba(234,88,12,.45),inset 0 1px 0 rgba(255,255,255,.22);text-shadow:0 1px 1px rgba(0,0,0,.25)}
#p .b0{background:linear-gradient(135deg,#1f1f3a,#13132e);border:1px solid #2a2a4a;color:#a0a0c8;box-shadow:0 1px 3px rgba(0,0,0,.3)}
#p .bh{padding:6px 8px;font-size:.66rem;border-radius:5px}
#p .tg{width:24px;height:12px;background:linear-gradient(180deg,#1a1a38,#0f0f24);border-radius:7px;border:1px solid #333366;cursor:pointer;position:relative;flex-shrink:0;transition:all .25s;box-shadow:inset 0 1px 2px rgba(0,0,0,.5)}
#p .tg-dot{width:8px;height:8px;background:linear-gradient(135deg,#888,#555);border-radius:50%;position:absolute;top:1px;left:1px;transition:all .25s;box-shadow:0 1px 2px rgba(0,0,0,.4)}
#p .tg.on{background:linear-gradient(180deg,#059669,#047857);border-color:#10b981;box-shadow:inset 0 1px 2px rgba(0,0,0,.3),0 0 6px rgba(16,185,129,.4)}
#p .tg.on .tg-dot{left:13px;background:linear-gradient(135deg,#fff,#a7f3d0);box-shadow:0 0 4px rgba(74,222,128,.6)}
#ps .sc{padding:4px 6px 4px}
#ps .su-section{background:linear-gradient(135deg,#13132e,#18183a);border-radius:6px;padding:4px 6px 3px;margin-bottom:1px;border:1px solid rgba(124,58,237,.15);box-shadow:none}
#ps .su-sec-title{display:none}
#ps .su-badge{font-size:.46rem;font-weight:800;padding:0px 4px;border-radius:2px;letter-spacing:.2px}
#ps .su-badge.on{background:rgba(16,185,129,.15);color:#34d399;border:1px solid rgba(52,211,153,.25)}
#ps .su-badge.off{background:rgba(239,68,68,.1);color:#f87171;border:1px solid rgba(248,113,113,.18)}
#ps .su-row{display:flex;gap:2px;margin-bottom:1px;align-items:center}
#ps .su-row input[type=text],#ps .su-row input[type=tel],#ps .su-row input[type=email],#ps .su-row input[type=password]{flex:1;min-width:0;background:linear-gradient(180deg,#1f1f3a,#181832);border:1px solid #3f3f65;padding:4px 6px;border-radius:4px;color:#eaeaff;font-size:.62rem;font-weight:600;outline:none;transition:all .2s}
#ps .su-row input::placeholder{color:#7575a0;font-size:.56rem;font-weight:500}
#ps .su-row input:focus{border-color:#7c3aed;box-shadow:0 0 0 1px rgba(124,58,237,.25)}
#ps .su-row button{padding:4px 6px;font-size:.58rem;font-weight:700;border-radius:4px;white-space:nowrap;flex:none;min-width:85px;text-align:center}
#ps .su-full{display:flex;gap:2px;margin-bottom:1px}
#ps .su-full button{flex:1;padding:4px 6px;font-size:.58rem;font-weight:700;border-radius:4px}
#ps .su-half{display:flex;gap:2px;margin-bottom:1px}
#ps .su-half button{flex:1;padding:4px 6px;font-size:.58rem;font-weight:700;border-radius:4px}
#ps .su-msg{font-size:.5rem;font-weight:600;min-height:10px;padding:0 2px 0px;margin-bottom:1px;line-height:1.2;border-radius:2px;transition:all .3s}
#ps .su-msg.g{color:#4ade80}
#ps .su-msg.r{color:#fca5a5}
#ps .su-msg.y{color:#fcd34d}
#ps .su-pw{flex:1;display:flex;position:relative;min-width:0}
#ps .su-pw input{width:100%;padding-right:20px!important}
#ps .su-pw .eye{position:absolute;right:2px;top:50%;transform:translateY(-50%);background:none;border:none;color:#8888aa;cursor:pointer;padding:0 3px;font-size:.64rem;line-height:1;z-index:2;transition:color .2s}
#ps .su-pw .eye:hover{color:#a78bfa}
#p .cp{background:linear-gradient(135deg,#070716,#0a0a1f);border-radius:6px;padding:6px;margin-top:4px;border:1px solid rgba(124,58,237,.2);box-shadow:inset 0 1px 3px rgba(0,0,0,.5)}
#p .cp .cr{display:flex;gap:3px;align-items:center;margin-bottom:5px}
#p .cp .cr .cl{display:flex;gap:3px;flex:1}
#p .cp .cr .cr2{display:flex;gap:3px;align-items:center;justify-content:flex-end}
#p .cp .cr button{padding:4px 6px;font-size:.6rem;font-weight:700;background:linear-gradient(135deg,#13132a,#0c0c1e);border:1px solid #1f1f3a;color:#777799;border-radius:4px;flex:1;text-align:center;transition:all .2s}
#p .cp .cr button:hover{background:linear-gradient(135deg,#1a1a3a,#13132a);color:#c0c0e0}
#p .cp .cr button.a{background:linear-gradient(135deg,#7c3aed,#4f46e5);border-color:#a78bfa;color:#fff;box-shadow:0 0 8px rgba(124,58,237,.45),inset 0 1px 0 rgba(255,255,255,.2)}
#p .cp .cf-wrap{width:100%;max-width:250px;margin:0 auto;border-radius:5px;overflow:hidden;background:linear-gradient(135deg,#0a0a1a,#06061a);border:1px solid rgba(124,58,237,.25);min-height:58px;display:flex;align-items:center;justify-content:center;margin-bottom:3px;box-shadow:inset 0 1px 4px rgba(0,0,0,.5)}
#p .cp .cf-wrap .cf-turnstile{width:100%}
#p .cp .cf-wrap iframe{border:none!important;border-radius:4px}
#p .cp .ck-row{display:flex;align-items:center;gap:0;margin-top:3px;padding:3px 5px;background:linear-gradient(90deg,#06061a,#0a0a1f,#06061a);border-radius:5px;border:1px solid rgba(124,58,237,.15)}
#p .cp .ck-time{font-size:.78rem;font-family:Consolas,monospace;font-weight:800;background:linear-gradient(90deg,#a78bfa,#c4b5fd);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;letter-spacing:.5px;flex-shrink:0}
#p .cp .ck-sep{width:1px;height:14px;background:linear-gradient(180deg,transparent,#3b3777,transparent);margin:0 7px;flex-shrink:0}
#p .cp .ck-date{font-size:.62rem;color:#8888aa;font-weight:600;flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
#p .cp .ck-row .b8{padding:3px 11px;border-radius:4px;font-size:.58rem;flex-shrink:0}
#p .ft{padding:10px 9px;background:linear-gradient(90deg,#06061a,#0a0a1f,#06061a);display:flex;justify-content:space-between;align-items:center;border-top:1px solid rgba(124,58,237,.2);flex-shrink:0}
#p .ft .fl{display:flex;gap:4px}
#p .ft button{background:linear-gradient(135deg,#0c0c1e,#13132a);border:1px solid #2a2a4a;padding:5px 12px;font-size:.68rem;border-radius:4px;color:#a0a0c8;font-weight:700;letter-spacing:.5px;transition:all .2s}
#p .ft button:hover{background:linear-gradient(135deg,#1a1a3a,#222248);color:#fff;border-color:#7c3aed;transform:translateY(-1px);box-shadow:0 2px 6px rgba(124,58,237,.3)}
#p .ft span{font-size:.58rem;background:linear-gradient(90deg,#7c3aed,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;font-weight:800;letter-spacing:1px}
#p .cdbar{background:linear-gradient(90deg,#05050f,#0a0a1f,#05050f);padding:6px 9px;display:flex;align-items:center;justify-content:center;gap:8px;border-top:1px solid rgba(124,58,237,.15);flex-shrink:0;min-height:26px;flex-wrap:nowrap}
#p .cd-empty{font-size:.6rem;color:#5a5a78;font-weight:600;letter-spacing:.4px;font-family:Consolas,monospace}
#p .cd-cell{display:flex;align-items:center;gap:5px;padding:3px 8px;border-radius:5px;background:linear-gradient(135deg,#0d0d24,#13132e);border:1px solid rgba(124,58,237,.25);box-shadow:inset 0 1px 2px rgba(0,0,0,.4);transition:all .3s}
#p .cd-cell .cd-icon{font-size:.7rem;line-height:1}
#p .cd-cell .cd-label{font-size:.56rem;color:#8888aa;font-weight:700;letter-spacing:.5px}
#p .cd-cell .cd-time{font-family:Consolas,monospace;font-size:.74rem;font-weight:800;letter-spacing:.5px}
#p .cd-otp .cd-time{color:#5eead4;text-shadow:0 0 6px rgba(94,234,212,.45)}
#p .cd-token .cd-time{color:#a78bfa;text-shadow:0 0 6px rgba(167,139,250,.45)}
#p .cd-sep{width:1px;height:14px;background:linear-gradient(180deg,transparent,#3b3777,transparent);flex-shrink:0}
#p .cd-cell.warn{border-color:#ef4444;background:linear-gradient(135deg,#1a0a0a,#2a0d0d);animation:cdpulse 1s ease-in-out infinite}
#p .cd-cell.warn .cd-time{color:#fca5a5 !important;text-shadow:0 0 8px rgba(252,165,165,.6) !important}
#p .cd-cell.warn .cd-label{color:#fca5a5}
@keyframes cdpulse{0%,100%{box-shadow:inset 0 1px 2px rgba(0,0,0,.4),0 0 6px rgba(239,68,68,.4)}50%{box-shadow:inset 0 1px 2px rgba(0,0,0,.4),0 0 14px rgba(239,68,68,.8)}}
#p .cd-cell.expired{border-color:#444;opacity:.55}
#p .cd-cell.expired .cd-time{color:#666 !important;text-shadow:none !important;text-decoration:line-through}
.btn-inline-cd{display:inline-block;margin-left:5px;padding:2px 6px;font-family:Consolas,monospace;font-size:.62rem;font-weight:800;border-radius:3px;background:rgba(0,0,0,.35);color:#fde047;letter-spacing:.4px}
.btn-inline-cd.warn{color:#fca5a5;background:rgba(239,68,68,.2);animation:cdpulse 1s ease-in-out infinite}
#p .tp{display:none}
#p .tp.a{display:block}
#pl input[type=text],#pl input[type=tel],#pl input[type=email],#pl input[type=password],#pl input[type=number],#pl select,#pf input[type=text],#pf input[type=number],#pf select,#pf textarea,#px input[type=text],#px input[type=number],#px input[type=password],#px select,#px textarea{background:linear-gradient(180deg,#15152e,#10102a) !important;border:1px solid #3a3a5e !important;color:#f0f0ff !important}
#pl input::placeholder,#pl select option,#pf input::placeholder,#pf textarea::placeholder,#px input::placeholder,#px textarea::placeholder{color:#7575a0 !important}
#pf .pl-row{display:flex;gap:5px;align-items:center;margin-bottom:5px}
#pf .pl-row span{font-size:.66rem;color:#b0b0cc;font-weight:700;white-space:nowrap}
#pf .pl-row input[type=number]{flex:1;text-align:center;padding:4px 2px;min-width:0}
#px textarea{height:58px}
#pu input[type=text],#pu input[type=tel],#pu input[type=email],#pu input[type=password],#pu input[type=number],#pu select,#pu input[type=file]{background:linear-gradient(180deg,#15152e,#10102a)!important;border:1px solid #3a3a5e!important;color:#f0f0ff!important}
#pu input::placeholder,#pu select option{color:#7575a0!important}#pu .fr{margin-bottom:4px;gap:4px}#pu button{padding:6px 7px!important;font-size:.64rem!important;line-height:1.25!important;border-radius:5px!important}#pu input[type=file]{font-size:.54rem!important;padding:3px!important;height:auto!important}#pu input[type=text],#pu input[type=number],#pu select{padding:6px 7px!important;font-size:.66rem!important}#pu .sc{padding:6px 9px 8px!important}
#px .proxy-msg{font-size:.62rem;color:#8888aa;font-weight:600;margin-top:3px;text-align:center}
#p .enc-section{margin-top:5px;padding:5px 6px;background:linear-gradient(135deg,#070716,#0a0a1f);border:1px dashed rgba(124,58,237,.3);border-radius:7px}
#p .enc-title{font-size:.64rem;color:#a78bfa;font-weight:800;letter-spacing:.5px;margin-bottom:4px}
#p .enc-row{display:flex;gap:4px;align-items:center;margin-bottom:3px}
#p .enc-row input[type=checkbox]{flex:none;accent-color:#7c3aed;width:14px;height:14px;cursor:pointer}
#p .enc-row label{flex:none;font-size:.62rem;color:#c4b5fd;font-weight:700;cursor:pointer;min-width:60px}
#p .enc-row .enc-status{flex:none;font-size:.54rem;font-weight:600;margin-left:auto}
#p .enc-editor{display:none;margin-bottom:4px}
#p .enc-editor textarea{height:100px;font-size:.58rem;line-height:1.35;background:linear-gradient(180deg,#0d0d24,#08081a)!important;border:1px solid rgba(124,58,237,.25)!important;color:#c4b5fd!important;border-radius:5px}
#p .enc-editor textarea::placeholder{color:#3a3a5e!important;font-size:.54rem}
#p .enc-editor .enc-btn-row{display:flex;gap:3px;margin-top:3px}
#p .enc-editor .enc-btn-row button{flex:1;padding:4px 0;font-size:.58rem;font-weight:700}
#p-fab{position:fixed;bottom:22px;left:22px;width:54px;height:54px;background:linear-gradient(135deg,#f97316,#ea580c,#c2410c);border-radius:50%;color:#fff;font-size:1.25rem;font-weight:900;z-index:9999999;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 20px rgba(249,115,22,.55),inset 0 2px 0 rgba(255,255,255,.25);cursor:pointer;transition:all .25s;border:2px solid rgba(255,255,255,.15);font-family:'Segoe UI',sans-serif;letter-spacing:.5px}
#p-fab:hover{transform:scale(1.12) rotate(-5deg);box-shadow:0 8px 28px rgba(249,115,22,.7),inset 0 2px 0 rgba(255,255,255,.3)}
#p-fab.hidden{display:none !important}
#profile-manager{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:370px;background:linear-gradient(160deg,#1a1a2e,#16162a,#13132e);border-radius:14px;box-shadow:0 20px 50px rgba(0,0,0,.85),0 0 0 1px rgba(124,58,237,.3),0 0 30px rgba(124,58,237,.12);z-index:99999999;font-family:'Segoe UI',system-ui,-apple-system,sans-serif;color:#e0e0ff;display:none;flex-direction:column;overflow:hidden}
#profile-manager.open{display:flex}
#profile-manager .pm-header{background:linear-gradient(90deg,#0a0a18,#15152e,#0a0a18);padding:10px 14px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(124,58,237,.3);user-select:none;cursor:move;position:relative}
#profile-manager .pm-header::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:linear-gradient(180deg,#7c3aed,#4f46e5);border-radius:0 2px 2px 0}
#profile-manager .pm-title{font-size:.78rem;font-weight:800;background:linear-gradient(90deg,#c4b5fd,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:.6px}
#profile-manager .pm-close{background:rgba(124,58,237,.1);border:1px solid rgba(124,58,237,.35);color:#a78bfa;font-size:.95rem;cursor:pointer;line-height:1;padding:0 8px;border-radius:5px;transition:all .2s}
#profile-manager .pm-close:hover{background:rgba(124,58,237,.25);color:#fff;transform:scale(1.05)}
#profile-manager .pm-body{overflow-y:auto;padding:12px;flex:1;background:#13132e}
#profile-manager .pm-section{background:linear-gradient(135deg,#1f1f38,#1a1a32);border-radius:9px;padding:10px 12px;margin-bottom:10px;border:1px solid rgba(124,58,237,.25)}
#profile-manager .pm-section h4{font-size:.68rem;font-weight:800;background:linear-gradient(90deg,#c4b5fd,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:9px;border-bottom:1px solid rgba(124,58,237,.3);padding-bottom:5px;text-transform:uppercase;letter-spacing:.6px}
#profile-manager .pm-select-row{display:flex;gap:7px;margin-bottom:11px;align-items:center}
#profile-manager .pm-select-row select{flex:2;background:linear-gradient(180deg,#0f0f24,#0a0a1c);border:1px solid #4a4a7a;color:#e0e0ff;padding:7px 9px;border-radius:7px;font-size:.72rem;font-weight:500}
#profile-manager .pm-select-row select:focus{border-color:#a78bfa;outline:none;box-shadow:0 0 0 2px rgba(167,139,250,.2)}
#profile-manager .pm-select-row button{padding:7px 11px;border-radius:7px;font-size:.66rem;font-weight:800;cursor:pointer;border:none;transition:all .2s;color:#fff;letter-spacing:.5px}
#profile-manager .pm-select-row button:hover{filter:brightness(1.15);transform:translateY(-1px)}
#profile-manager .pm-btn-new{background:linear-gradient(135deg,#10b981,#059669);box-shadow:0 2px 8px rgba(16,185,129,.4);flex:1}
#profile-manager .pm-btn-del{background:linear-gradient(135deg,#ef4444,#b91c1c);box-shadow:0 2px 8px rgba(239,68,68,.4);flex:1}
#profile-manager .pm-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-bottom:7px}
#profile-manager input{background:linear-gradient(180deg,#0f0f24,#0a0a1c);border:1px solid #4a4a7a;color:#e0e0ff;padding:7px 9px;border-radius:7px;font-size:.72rem;outline:none;box-sizing:border-box;font-family:inherit;font-weight:500;transition:all .2s}
#profile-manager input:focus{border-color:#a78bfa;box-shadow:0 0 0 2px rgba(167,139,250,.2)}
#profile-manager input::placeholder{color:#6a6a9a;font-size:.66rem;font-weight:400}
#profile-manager .pm-action-row{display:flex;gap:7px;margin-top:9px}
#profile-manager .pm-action-row button{flex:1;padding:8px;border-radius:7px;font-size:.7rem;font-weight:800;cursor:pointer;border:none;transition:all .2s;color:#fff;letter-spacing:.5px}
#profile-manager .pm-action-row button:hover{filter:brightness(1.15);transform:translateY(-1px)}
#profile-manager .pm-btn-export{background:linear-gradient(135deg,#3b82f6,#1d4ed8);box-shadow:0 2px 8px rgba(59,130,246,.4)}
#profile-manager .pm-btn-import{background:linear-gradient(135deg,#8b5cf6,#6d28d9);box-shadow:0 2px 8px rgba(139,92,246,.4)}
#profile-manager .pm-btn-save{background:linear-gradient(135deg,#059669,#047857);box-shadow:0 2px 8px rgba(5,150,105,.4)}
#pm-import-file{display:none}
#p #bst,#p .cp .cr .cl button#csi,#p .cp .cr .cl button#csr,#p #pl-button,#p #badv,#p #bsgn{padding-top:4px !important;padding-bottom:4px !important;line-height:1.15 !important}
#p .tq-row{display:flex;align-items:center;gap:4px;margin-top:2px;padding:2px 5px;background:linear-gradient(90deg,#06061a,#0a0a1f,#06061a);border-radius:4px;border:1px solid rgba(124,58,237,.15);font-size:.54rem;color:#8888aa;font-weight:600}
#p .tq-label{flex-shrink:0;font-size:.54rem;letter-spacing:.3px}
#p .tq-slots{display:flex;gap:2px;flex:1;justify-content:center}
#p .tq-slot{width:10px;height:10px;border-radius:2px;border:1px solid #2a2a4a;background:linear-gradient(135deg,#0a0a18,#06061a);transition:all .3s;position:relative}
#p .tq-slot.empty{background:linear-gradient(135deg,#0a0a18,#06061a);border-color:#2a2a4a;opacity:.4}
#p .tq-slot.turnstile{background:linear-gradient(135deg,#06b6d4,#0891b2);border-color:#22d3ee;box-shadow:0 0 4px rgba(34,211,238,.5),inset 0 1px 0 rgba(255,255,255,.2);opacity:1}
#p .tq-slot.capmonster{background:linear-gradient(135deg,#a855f7,#7e22ce);border-color:#c084fc;box-shadow:0 0 4px rgba(168,85,247,.5),inset 0 1px 0 rgba(255,255,255,.2);opacity:1}
#p .tq-slot.capsolver{background:linear-gradient(135deg,#10b981,#059669);border-color:#34d399;box-shadow:0 0 4px rgba(52,211,153,.5),inset 0 1px 0 rgba(255,255,255,.2);opacity:1}
#p .tq-slot.twocaptcha{background:linear-gradient(135deg,#f59e0b,#d97706);border-color:#fbbf24;box-shadow:0 0 4px rgba(251,191,36,.5),inset 0 1px 0 rgba(255,255,255,.2);opacity:1}
#p .tq-slot.yescaptcha{background:linear-gradient(135deg,#ec4899,#be185d);border-color:#f472b6;box-shadow:0 0 4px rgba(244,114,182,.5),inset 0 1px 0 rgba(255,255,255,.2);opacity:1}
#p .tq-slot.solving{background:linear-gradient(135deg,#fcd34d,#f59e0b);border-color:#fcd34d;box-shadow:0 0 6px rgba(252,211,77,.6);animation:tqPulse 1s ease-in-out infinite}
#p .tq-slot.expiring{animation:tqExpiring 1s ease-in-out infinite}
@keyframes tqPulse{0%,100%{box-shadow:0 0 4px rgba(252,211,77,.5)}50%{box-shadow:0 0 10px rgba(252,211,77,.9)}}
@keyframes tqExpiring{0%,100%{filter:brightness(1)}50%{filter:brightness(0.5) hue-rotate(-30deg)}}
#p .tq-info{flex-shrink:0;font-family:Consolas,monospace;font-size:.5rem;color:#a78bfa;font-weight:700;min-width:62px;text-align:right;letter-spacing:.2px}
#p .tq-info.solving{color:#fcd34d;animation:tqInfoPulse 1.5s ease-in-out infinite}
@keyframes tqInfoPulse{0%,100%{opacity:.7}50%{opacity:1}}
#rj-netlog{position:fixed;bottom:90px;left:20px;width:480px;max-width:90vw;height:420px;background:linear-gradient(160deg,#0d0d1e,#10102a,#0a0a18);border-radius:12px;box-shadow:0 20px 50px rgba(0,0,0,.85),0 0 0 1px rgba(124,58,237,.3),0 0 25px rgba(124,58,237,.15);z-index:99999990;display:none;flex-direction:column;font-family:'Segoe UI',system-ui,sans-serif;color:#e0e0f0;overflow:hidden;backdrop-filter:blur(8px)}
#rj-netlog.open{display:flex}
#rj-netlog .netlog-header{background:linear-gradient(90deg,#0a0a18,#15152e,#0a0a18);padding:9px 13px;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(124,58,237,.3);cursor:move;user-select:none;position:relative}
#rj-netlog .netlog-header::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:linear-gradient(180deg,#7c3aed,#4f46e5);border-radius:0 2px 2px 0}
#rj-netlog .netlog-title{font-size:.78rem;font-weight:800;background:linear-gradient(90deg,#c4b5fd,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:.6px}
#rj-netlog .netlog-tools{display:flex;gap:6px}
#rj-netlog .netlog-tools button{background:linear-gradient(135deg,#13132e,#1f1f3a);border:1px solid rgba(124,58,237,.3);color:#c4b5fd;font-size:.66rem;font-weight:700;padding:3px 10px;border-radius:5px;cursor:pointer;transition:all .2s}
#rj-netlog .netlog-tools button:hover{background:linear-gradient(135deg,#7c3aed,#4f46e5);color:#fff}
#rj-netlog .netlog-tools #netlog-close{padding:3px 9px}
#rj-netlog .netlog-meta{padding:6px 12px;background:#06061a;display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid rgba(124,58,237,.15);font-size:.62rem;color:#8888aa;font-weight:600}
#rj-netlog .netlog-steps{padding:8px 10px;background:linear-gradient(90deg,#06061a,#0a0a1f,#06061a);display:flex;gap:6px;border-bottom:1px solid rgba(124,58,237,.15)}
#rj-netlog .nl-step{flex:1;text-align:center;font-size:.66rem;font-weight:800;padding:6px 4px;border-radius:6px;background:linear-gradient(135deg,#1a1a2e,#13132a);border:1px solid #2a2a4a;color:#7777aa;letter-spacing:.3px;transition:all .3s;position:relative}
#rj-netlog .nl-step.active{background:linear-gradient(135deg,#fcd34d,#f59e0b);border-color:#fcd34d;color:#1a1a00;box-shadow:0 0 10px rgba(252,211,77,.5),inset 0 1px 0 rgba(255,255,255,.3);animation:nlPulse 1s ease-in-out infinite}
#rj-netlog .nl-step.success{background:linear-gradient(135deg,#10b981,#059669);border-color:#34d399;color:#fff;box-shadow:0 0 12px rgba(16,185,129,.55),inset 0 1px 0 rgba(255,255,255,.25)}
#rj-netlog .nl-step.fail{background:linear-gradient(135deg,#ef4444,#b91c1c);border-color:#f87171;color:#fff;box-shadow:0 0 10px rgba(239,68,68,.5),inset 0 1px 0 rgba(255,255,255,.25)}
@keyframes nlPulse{0%,100%{box-shadow:0 0 10px rgba(252,211,77,.4)}50%{box-shadow:0 0 18px rgba(252,211,77,.7)}}
#rj-netlog #netlog-filter{background:#0a0a1a;border:1px solid #2a2a4a;color:#c4b5fd;font-size:.6rem;padding:2px 4px;border-radius:4px;cursor:pointer;font-family:inherit}
#rj-netlog .netlog-body{flex:1;overflow-y:auto;padding:6px;background:#06061a;font-family:Consolas,'Courier New',monospace;font-size:.66rem;line-height:1.5}
#rj-netlog .netlog-body::-webkit-scrollbar{width:6px}
#rj-netlog .netlog-body::-webkit-scrollbar-thumb{background:rgba(124,58,237,.4);border-radius:3px}
#rj-netlog .nl-entry{padding:4px 8px;margin-bottom:3px;border-radius:5px;border-left:3px solid #2a2a4a;background:rgba(15,15,35,.7);display:grid;grid-template-columns:65px 55px 60px 1fr;gap:6px;align-items:center}
#rj-netlog .nl-entry.ok{border-left-color:#10b981}
#rj-netlog .nl-entry.fail{border-left-color:#ef4444;background:rgba(40,15,15,.55)}
#rj-netlog .nl-entry.pending{border-left-color:#fcd34d;background:rgba(30,28,5,.55)}
#rj-netlog .nl-entry.cancel{border-left-color:#888;opacity:.55}
#rj-netlog .nl-time{color:#8888aa;font-size:.6rem}
#rj-netlog .nl-method{color:#a78bfa;font-weight:700;font-size:.6rem}
#rj-netlog .nl-status{font-weight:800;font-size:.62rem;text-align:center;padding:1px 4px;border-radius:3px;background:#13132e}
#rj-netlog .nl-status.ok{background:rgba(16,185,129,.2);color:#34d399}
#rj-netlog .nl-status.fail{background:rgba(239,68,68,.2);color:#fca5a5}
#rj-netlog .nl-status.pending{background:rgba(252,211,77,.2);color:#fcd34d}
#rj-netlog .nl-status.cancel{background:rgba(136,136,136,.2);color:#aaa}
#rj-netlog .nl-url{color:#e0e0f0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:.62rem}
#rj-netlog .nl-tag{display:inline-block;font-size:.55rem;font-weight:700;padding:1px 4px;border-radius:3px;background:rgba(124,58,237,.25);color:#c4b5fd;margin-right:4px;text-transform:uppercase}
#rj-netlog .nl-empty{text-align:center;color:#555588;padding:30px 10px;font-size:.7rem;font-family:inherit}
#rj-milestone-overlay{position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999999999;display:flex;align-items:center;justify-content:center;backdrop-filter:blur(3px);font-family:'Segoe UI',system-ui,sans-serif}
#rj-milestone-overlay .mp-card{width:300px;background:linear-gradient(160deg,#1a1a2e,#16162a);border-radius:14px;box-shadow:0 25px 60px rgba(0,0,0,.85),0 0 0 1px rgba(124,58,237,.3),0 0 30px rgba(124,58,237,.15);overflow:hidden;text-align:center;animation:mpFadeIn .25s ease-out}
#rj-milestone-overlay .mp-emoji{padding:22px 20px 10px;font-size:2.2rem}
#rj-milestone-overlay .mp-title{padding:0 20px 6px;color:#fff;font-size:.88rem;font-weight:800;letter-spacing:.3px}
#rj-milestone-overlay .mp-msg{padding:0 20px 16px;color:#b0b0d0;font-size:.72rem;font-weight:600;line-height:1.5;word-break:break-all}
#rj-milestone-overlay .mp-ok{width:100%;background:linear-gradient(135deg,#7c3aed,#4f46e5);border:1px solid #a78bfa;color:#fff;padding:10px 0;border-radius:8px;font-size:.76rem;font-weight:800;cursor:pointer;letter-spacing:.5px;box-shadow:0 3px 10px rgba(124,58,237,.4);transition:all .2s}
#rj-milestone-overlay .mp-ok:hover{filter:brightness(1.15);transform:translateY(-1px)}
@keyframes mpFadeIn{from{opacity:0;transform:scale(.9)}to{opacity:1;transform:scale(1)}}
`;
...
<div id="p">
  ...
  <div class="tb">
    ...
    <button class="t" data-t="s">Sign Up</button>
    <button class="t" data-t="u">Upload</button>
    <button class="t" data-t="f">Fetch</button>
    <button class="t" data-t="x">Proxy</button>
  </div>
  <div class="bd">
    <div class="sl">
    ...
    <div class="tp a" id="pl">
      <div class="sc">
        ...
        <div class="fr sa-row">
          <input type="number" id="rt-signin"   value="1"  min="0" max="999" title="Signin retry delay (seconds)">
          <input type="number" id="rt-verify"   value="1"  min="0" max="999" title="Verify retry delay (seconds)">
          <input type="number" id="rt-reserve"  value="21" min="0" max="999" title="Reserve retry delay (seconds)">
          <input type="number" id="rt-book"     value="1"  min="0" max="999" title="Book retry delay (seconds)">
          <input type="number" id="rt-initiate" value="1"  min="0" max="999" title="Initiate retry delay (seconds)">
          <button class="b5 toggle-btn sa-btn" id="btn-single" title="Single ON: retry on failure. OFF: no retry, fail = stop">Single</button>
          <input type="number" id="rt-auto-loops" value="0" min="0" max="999" title="Reserved (currently unused)">
          <button class="b5 toggle-btn sa-btn" id="btn-auto" title="Auto ON: after a step wins, automatically continues to next step. Manual button starts the flow.">Auto</button>
        </div>
        <div class="fr-phone">
          <select id="login-phone-type" class="phone-type-sel">
            <option value="phone1">📞 P1</option>
            <option value="email">✉️ Em</option>
            <option value="phone2">📞 P2</option>
          </select>
          <input type="tel" id="login-phone" placeholder="Phone 1">
          <div class="sw-c">
            <div class="tg" id="advance-toggle"><div class="tg-dot"></div></div>
          </div>
          <button class="b2 bh" style="padding:4px 8px!important" id="badv-phone">Advance</button>
        </div>
        ...
        <div class="fr">
          <button class="b1 bh" style="flex:1" id="brs">Reserve</button>
          <button class="b14 bh" style="flex:1" id="bbk">Book</button>
          <button class="b6 bh" style="flex:1" id="bin">Initiate</button>
        </div>
        ...
        <div class="cp">
          <div class="cr">
            ...
            <div class="cr2">
              ...
              <div class="tg" id="parallel-toggle" style="margin-left:5px" title="Parallel mode (leaky-bucket retries)"><div class="tg-dot"></div></div>
              <button id="pl-button" style="margin-left:5px;background:linear-gradient(135deg,#7c3aed,#4f46e5);border:1px solid #a78bfa;color:#fff;padding:4px 10px;border-radius:5px;font-size:0.6rem;font-weight:800;box-shadow:0 2px 6px rgba(124,58,237,.4)">A_L</button>
          ...
          <div class="tq-row" id="tq-row" title="Captcha token queue status">
            <span class="tq-label">🎫 Queue</span>
            <span class="tq-slots" id="tq-slots">
              <span class="tq-slot empty"></span>
              <span class="tq-slot empty"></span>
              <span class="tq-slot empty"></span>
              <span class="tq-slot empty"></span>
              <span class="tq-slot empty"></span>
            </span>
            <span class="tq-info" id="tq-info">0/5 • idle</span>
          </div>
          <div class="ck-row">
            <span class="ck-time" id="tk">12:13:17 AM</span>
            <span class="ck-sep"></span>
            <span class="ck-date" id="dk">08/05/2026</span>
            <button class="b8" id="blo">Logout</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="tp" id="pf">
      <div class="sc">
        <div class="enc-section">
          <div class="enc-title">🔐 Unified Token Encryption (Signin + Reserve)</div>
          <div class="enc-row"><input type="checkbox" id="enc-unified-check"><label for="enc-unified-check">Enable Encryption</label><button class="b7 bh" id="enc-unified-btn" style="flex:0 0 auto;padding:3px 8px;font-size:.58rem">Edit Script</button><span class="enc-status" id="enc-unified-status">Hardcode</span></div>
          <div class="enc-editor" id="enc-unified-editor"><textarea id="enc-unified-script" placeholder='Paste your unified encryption script here...'></textarea><div class="enc-btn-row"><button class="b5" id="enc-unified-save">💾 Save</button><button class="b8" id="enc-unified-clear">✕ Clear</button></div></div>
        </div>
        <textarea id="ivac-fetch-input" rows="3" placeholder='{"url":"...", "method":"POST", "headers":{}, "body":null}'></textarea>
  ...
  <div class="ft">
    ...
    <div class="tg" id="signin-timeout-toggle" title="Signin Timeout: ON = 20s timeout, force retry" style="width:28px;height:16px;margin-left:6px;flex-shrink:0"><div class="tg-dot"></div></div>
  </div>
...
<div id="rj-netlog">
  ...
  <div class="netlog-meta">
    <span id="netlog-stats">0 entries • auto-clear in 3:00</span>
    <span id="netlog-filter-wrap"><select id="netlog-filter"><option value="">All</option><option value="signup">Sign Up</option><option value="signin">Signin</option><option value="verify">Verify</option><option value="reserve">Reserve</option><option value="book">Book</option><option value="initiate">Initiate</option><option value="captcha">Captcha</option><option value="sms">SMS</option><option value="slot">Slot Status</option><option value="upload">Upload</option><option value="invoice">Invoice</option><option value="advance">Advance</option><option value="network">Network</option></select></span>
  </div>
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">`;
    ...
    document.getElementById('enc-unified-btn')?.addEventListener('click', () => {
        const editor = document.getElementById('enc-unified-editor');
        if (editor) editor.style.display = editor.style.display === 'block' ? 'none' : 'block';
    });
    document.getElementById('enc-unified-save')?.addEventListener('click', () => {
        const script = document.getElementById('enc-unified-script')?.value;
        encManager.saveUnifiedScript(script);
        if (encManager.loadUnifiedScript(script)) { logStatus('✅ Unified encryption saved & loaded', 'g'); }
        else { logStatus('❌ Unified encryption script parse error', 'r'); }
        encManager.updateStatusUI();
    });
    document.getElementById('enc-unified-clear')?.addEventListener('click', () => {
        encManager._unifiedFn = null;
        encManager.saveUnifiedScript('');
        if (document.getElementById('enc-unified-script')) document.getElementById('enc-unified-script').value = '';
        if (document.getElementById('enc-unified-check')) document.getElementById('enc-unified-check').checked = false;
        encManager.saveEnabledCheck(false);
        encManager.updateStatusUI();
        logStatus('⚠ Unified encryption cleared — falling back to Hardcode', 'y');
    });
    document.getElementById('enc-unified-check')?.addEventListener('change', (e) => {
        encManager.saveEnabledCheck(e.target.checked);
    ...
    const lmap = { l: 'll', s: 'ls', u: 'lu', f: 'lf' };
    ...
    document.querySelectorAll('#p .pw .eye').forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            if (inp) { inp.type = inp.type === 'password' ? 'text' : 'password'; this.innerHTML = inp.type === 'password' ? '&#128065;' : '&#128526;'; }
        });
    });

    // ==================== TOGGLE SWITCHES ====================
    document.querySelectorAll('#p .tg').forEach(el => { el.addEventListener('click', () => el.classList.toggle('on')); });
    ...
    document.getElementById('advance-toggle')?.addEventListener('click', () => {
        setTimeout(() => {
            if (typeof applyAdvanceModeToLoginField === 'function') {
                ...
                logStatus(advanceOn ? '🔑 Advance mode ON — login field switched to Email (Forgot mode)' : '🔑 Advance mode OFF — login field switched to Phone (Signin mode)', advanceOn ? 'g' : 'y');
            }
        }, 0);
    });
    ...
    function refreshProfileSelects() {
        if (pmSelect) {
            pmSelect.innerHTML = '';
            Object.keys(profiles).forEach(n => { const o = document.createElement('option'); o.value = n; o.textContent = n; if (n === activeProfileName) o.selected = true; pmSelect.appendChild(o); });
        }
        if (pmSelectMain) {
            pmSelectMain.innerHTML = '<option value="">-- Select Profile --</option>';
            Object.keys(profiles).forEach(n => { const o = document.createElement('option'); o.value = n; o.textContent = n; if (n === activeProfileName) o.selected = true; pmSelectMain.appendChild(o); });
        }
    ...
    function applyAdvanceModeToLoginField() {
        ...
        if (advanceOn) {
            loginPhoneInp.type = 'email'; loginPhoneInp.placeholder = 'Email'; loginPhoneInp.value = p.email || ''; loginPhoneInp.title = 'Email (Forgot Password mode)';
        } else {
            loginPhoneInp.type = 'tel'; loginPhoneInp.placeholder = 'Phone 1'; loginPhoneInp.value = p.phone1 || ''; loginPhoneInp.title = 'Phone (Signin mode)';
        }
    }
    ...
    function loadProfileToForm(name) {
        ...
        try {
            ...
            if (p.name) { const nameParts = p.name.split(' '); if (suSurname) suSurname.value = nameParts.length > 1 ? nameParts.slice(-1)[0] : p.name; if (suGivenName) suGivenName.value = nameParts.length > 1 ? nameParts.slice(0,-1).join(' ') : ''; }
            else { if (suSurname) suSurname.value = ''; if (suGivenName) suGivenName.value = ''; }
    ...
    function saveFormToProfile() {
        profiles[activeProfileName] = {
            name: pmName?.value || '', email: pmEmail?.value || '', phone1: pmPhone1?.value || '', phone2: pmPhone2?.value || '',
            mobilePass: pmMobilePass?.value || '', emailPass: pmEmailPass?.value || '',
    ...
    pmImportInput?.addEventListener('change', e => {
        ...
        r.onload = ev => {
            try { const imported = JSON.parse(ev.target.result); profiles = { ...profiles, ...imported }; persistProfiles(); refreshProfileSelects(); logStatus(`✓ Imported ${Object.keys(imported).length} profile(s)`, 'g'); }
    ...
    // "A" button next to A_L — delete saved Appointment ID from active profile + session
    ...
    function playBeepSequence(beeps) { let offset = 0; beeps.forEach(b => { setTimeout(() => playBeep(b.freq, b.durationMs, b.volume), offset); offset += b.durationMs + 70; }); }
    ...
    function beepInitiateAndSpeak() {
        playBeepSequence([{ freq: 523.25, durationMs: 180, volume: 0.10 }, { freq: 659.25, durationMs: 180, volume: 0.10 }, { freq: 783.99, durationMs: 350, volume: 0.12 }]);
    ...
    function attachInlineCd(btn, slotKey) { if (!btn) return null; let span = btn.querySelector('.btn-inline-cd'); if (!span) { span = document.createElement('span'); span.className = 'btn-inline-cd'; btn.appendChild(span); } return span; }
    function clearInlineCd(btn) { const span = btn?.querySelector('.btn-inline-cd'); if (span) span.remove(); }

    function tickOtpTimer(slotKey) {
        const t = timers[slotKey]; const btn = document.getElementById(t.btnId);
        if (!btn || !t.expiresAt) return;
        const remain = t.expiresAt - Date.now(); const span = attachInlineCd(btn, slotKey); if (!span) return;
        if (remain <= 0) { clearInterval(t.intervalId); t.intervalId = null; t.expiresAt = null; t.beeped = false; clearInlineCd(btn); logStatus(`⌛ ${slotKey === 'signinOtp' ? 'Signin' : 'Advance'} OTP expired`, 'r'); return; }
        span.textContent = fmtRemaining(remain);
        if (remain <= WARN_THRESHOLD_MS) { span.classList.add('warn'); if (!t.beeped) { beepSoft(); t.beeped = true; } } else { span.classList.remove('warn'); }
    }

    function startOtpTimer(slotKey) { const t = timers[slotKey]; if (t.intervalId) clearInterval(t.intervalId); t.expiresAt = Date.now() + TIMER_OTP_MS; t.beeped = false; tickOtpTimer(slotKey); t.intervalId = setInterval(() => tickOtpTimer(slotKey), 1000); }
    function stopOtpTimer(slotKey) { const t = timers[slotKey]; if (t.intervalId) { clearInterval(t.intervalId); t.intervalId = null; } t.expiresAt = null; t.beeped = false; const btn = document.getElementById(t.btnId); if (btn) clearInlineCd(btn); }

    function refreshTokenCdUI() {
        const t = timers.token;
        const cdbar = document.getElementById('cdbar'); const empty = document.getElementById('cd-empty');
        const otpCell = document.getElementById('cd-otp'); const tokenCell = document.getElementById('cd-token');
        const sep = document.getElementById('cd-sep'); const tokenTime = document.getElementById('cd-token-time');
        ...
        if (anyOtpActive) {
            const otpRemain = Math.max(timers.signinOtp.expiresAt ? timers.signinOtp.expiresAt - Date.now() : 0, timers.advanceOtp.expiresAt ? timers.advanceOtp.expiresAt - Date.now() : 0);
            const otpTimeEl = document.getElementById('cd-otp-time'); if (otpTimeEl) otpTimeEl.textContent = fmtRemaining(otpRemain);
            if (otpCell) { if (otpRemain <= WARN_THRESHOLD_MS) otpCell.classList.add('warn'); else otpCell.classList.remove('warn'); }
        }
        if (!t.expiresAt) { if (cdbar && !anyOtpActive) cdbar.style.display = 'none'; if (empty && !anyOtpActive) empty.style.display = ''; if (tokenCell) tokenCell.style.display = 'none'; return; }
        const remain = t.expiresAt - Date.now();
        if (cdbar) cdbar.style.display = ''; if (empty) empty.style.display = 'none'; if (tokenCell) tokenCell.style.display = '';
        if (remain <= 0) {
            tokenTime.textContent = '00:00'; tokenCell.classList.remove('warn'); tokenCell.classList.add('expired');
            ...
            try { clearAllSession(); } catch(e) {} try { stopOtpTimer('signinOtp'); stopOtpTimer('advanceOtp'); } catch(e) {}
            setTimeout(() => { tokenCell.classList.remove('expired'); refreshTokenCdUI(); }, 3000); return;
        }
        tokenTime.textContent = fmtRemaining(remain);
        if (remain <= WARN_THRESHOLD_MS) { tokenCell.classList.add('warn'); if (!t.beeped) { beepSoft(); t.beeped = true; } } else { tokenCell.classList.remove('warn'); }
    }

    function startTokenTimer() { const t = timers.token; if (t.intervalId) clearInterval(t.intervalId); t.expiresAt = Date.now() + TIMER_TOKEN_MS; t.beeped = false; refreshTokenCdUI(); t.intervalId = setInterval(refreshTokenCdUI, 1000); }
    function startTokenTimerWithExpiry(expiresAt) { const t = timers.token; if (t.intervalId) clearInterval(t.intervalId); t.expiresAt = expiresAt; t.beeped = false; refreshTokenCdUI(); t.intervalId = setInterval(refreshTokenCdUI, 1000); }
    refreshTokenCdUI();
    ...
    function flashButton(btn, statusEmoji, color, durationMs) {
        ...
        btn.textContent = statusEmoji; if (cdSpan) btn.appendChild(cdSpan);
        ...
        setTimeout(() => { btn.textContent = original; if (cdSpan) btn.appendChild(cdSpan); btn.style.background = ''; btn.style.borderColor = ''; }, durationMs || 1500);
    }
    ...
    function formatProxyString(p) { if (!p) return 'system'; const auth = p.user ? `${p.user}${p.password ? ':' + p.password : ''}@` : ''; return `${p.scheme}://${auth}${p.host}:${p.port}`; }

    function refreshProxyPicker() {
        const list = loadProxies(); const activeId = getActiveProxyId();
        const pickers = [ { el: document.getElementById('ivac-proxy-picker'),  emptyLabel: '-- Saved proxies --' }, { el: document.getElementById('login-proxy-picker'), emptyLabel: '-- Direct (no proxy) --' } ];
    ...
    function rjPickProxyForRequest(isConnectionCheck) {
        if (isConnectionCheck) return null;
        if (rjRotationEnabled()) {
            const pool = rjRotationPool();
            if (!pool.length) return (typeof window !== 'undefined') ? (window._rjActiveProxy || null) : null;
            if (!rjRot.current) {
    ...
    function rjRotationFeedback(win) {
        ...
        if (win) { rjRot.lastFailedId = null; }
        else { if (rjRot.current) rjRot.lastFailedId = rjRot.current.id; rjRot.current = null; }
    }
    ...
    async function uploadFile(fileInputId, isPrimary, label) {
        ...
        let uploadCaptcha; try { const t = await getCaptchaTokenSmart(); uploadCaptcha = encManager.encryptToken(t, 'Signin'); } catch(e) { logStatus(`❌ ${label} captcha: ${e.message}`, 'r'); return; }
        formData.append('c', uploadCaptcha);
        logStatus(`📄 Uploading ${label}…`, 'y');
        const logId = netLogAdd({ method: 'POST', url: API_UPLOAD_FILE, tag: 'upload', state: 'pending', note: `${label}` });
        try {
            // Current bundle (2026-08-13): endpoint renamed to /file/upload_file_v2232 and the encrypted
            // captcha is also sent as the x-token header (kept in the 'c' form field too for compatibility).
            const uploadHeaders = { 'accept': 'application/json', 'authorization': `Bearer ${sessionState.accessToken}`, 'x-device-id': getDeviceId(), 'x-token': uploadCaptcha };
            const secState = getSecRuntimeState(); if (secState) uploadHeaders['x-sec-runtime-state'] = secState;
            const r = await H2.fetchH2Upload(API_UPLOAD_FILE, { method: 'POST', headers: uploadHeaders, referrer: API_REFERRER, body: formData });
            let body = null; try { body = await r.json(); } catch(e) { body = null; }
            const ok = r.ok && body && (body.successFlag === true || body.statusCode === 200);
    ...
    document.getElementById('ivac-btn-file-checking')?.addEventListener('click', async function() {
        ...
        const logId = netLogAdd({ method: 'POST', url: API_FILE_OVERVIEW, tag: 'upload', state: 'pending' });
        try {
            const r = await H2.fetchH2(API_FILE_OVERVIEW, { method: 'POST', headers: { 'accept': 'application/json', 'authorization': `Bearer ${sessionState.accessToken}`, 'x-device-id': getDeviceId() }, referrer: API_REFERRER, body: null });
            let body = null; try { body = await r.json(); } catch(e) { body = null; }
            const ok = r.ok && body && (body.successFlag === true || body.statusCode === 200);
    ...
    document.getElementById('ivac-btn-mobile')?.addEventListener('click', async function() {
        ...
        try {
            const r = await H2.fetchH2("https://api.ivacbd.com/iams/api/v1/otp/signupOtp", { method: 'POST', headers: { 'accept': 'application/json', 'content-type': 'application/json' }, referrer: "https://appointment.ivacbd.com/", body: JSON.stringify({ phone, email, otpChannel: "PHONE" }) });
    ...
    document.getElementById('ivac-btn-email')?.addEventListener('click', async function() {
        ...
        try {
            const r = await H2.fetchH2("https://api.ivacbd.com/iams/api/v1/otp/signupOtp", { method: 'POST', headers: { 'accept': 'application/json, text/plain, */*', 'content-type': 'application/json', 'cache-control': 'no-cache, no-store, must-revalidate', 'pragma': 'no-cache', 'x-device-id': getDeviceId() }, referrer: "https://appointment.ivacbd.com/", body: JSON.stringify({ email, otpChannel: "EMAIL" }) });
            let body = null; try { body = await r.json(); } catch(e) {}
            const ok = r.ok && body && (body.successFlag === true || body.statusCode === 200);
            netLogUpdate(logId, { status: r.status, state: ok ? 'ok' : 'fail', note: ok ? `requestId=${body?.data?.requestId?.slice(0,8)||'?'}` : (body?.message || `HTTP ${r.status}`) });
    ...
    document.getElementById('ivac-btn-submit-info')?.addEventListener('click', async function() {
        ...
        let signupCaptcha; try { const t = await getCaptchaTokenSmart(); signupCaptcha = encManager.encryptToken(t, 'Signin'); } catch(e) { suMsg('ivac-msg-submit', `❌ Captcha: ${e.message}`, 'r'); flashButton(btn, '✗', 'r'); return; }
        const signupBody = { phone, email, nid, passport, givenName, surName, dob, password, c: signupCaptcha };
        console.log('[RJ Signup] Request body:', { ...signupBody, password: '***', c: '***' });
    ...
    async function forgotStep1_sendOtp(signal) {
        ...
        try {
            const r = await H2.fetchH2(API_FORGOT, { method: 'POST', signal: localAc.signal, headers: { 'accept': 'application/json', 'content-type': 'application/json', 'x-device-id': getDeviceId() }, referrer: API_REFERRER, body: JSON.stringify({ email: prof.email, otpChannel: 'PHONE', c: encManager.encryptToken(captchaToken, 'Signin') }) });
    ...
    async function forgotStep4_verify(signal) {
        ...
        try {
            const r = await H2.fetchH2(API_VERIFY, { method: 'POST', signal, headers: { 'accept': 'application/json', 'authorization': `Bearer ${advanceState.bearerToken}`, 'content-type': 'application/json', 'x-device-id': getDeviceId() }, referrer: API_REFERRER, body: JSON.stringify({ requestId: advanceState.requestId, phone: advanceState.forgotPhone, code: advanceState.otp, otpChannel: 'PHONE' }) });
            let body = null; try { body = await r.json(); } catch(e) { body = null; }
    ...
    async function fetchSmsOnce(phone) {
        ...
        try { const r = await pageFetch(url, { method: 'GET', credentials: 'omit' }); const text = await r.text(); const otp = extractOtpFromResponse(text); netLogUpdate(logId, { state: otp ? 'ok' : 'pending', note: otp ? `OTP: ${otp}` : 'no OTP' }); return otp; }
        catch(e) {
            return new Promise((resolve) => {
                const gmApi = (typeof GM_xmlhttpRequest !== 'undefined' && GM_xmlhttpRequest) || (typeof GM !== 'undefined' && GM.xmlHttpRequest);
    ...
    // ==================== TOKEN QUEUE ====================
    const TOKEN_TTL_MS = 4 * 60 * 1000; const TOKEN_QUEUE_MAX = 5; const tokenQueue = [];
    ...
    setInterval(renderTokenQueue, 1000); setTimeout(renderTokenQueue, 200);
    document.getElementById('captcha-toggle')?.addEventListener('click', () => { setTimeout(renderTokenQueue, 50); });
    ...
    document.getElementById('ivac-captcha-api-input')?.addEventListener('input', (e) => { const v = (e.target.value || '').trim(); if (v) { const provider = getSelectedCaptchaProvider(); const config = CAPTCHA_PROVIDERS[provider]; if (config) localStorage.setItem(config.keyStorage, v); } });
    document.getElementById('ivac-btn-captcha-save')?.addEventListener('click', () => { const k = (document.getElementById('ivac-captcha-api-input')?.value || '').trim(); if (!k) { logStatus('❌ API key empty', 'r'); return; } const provider = getSelectedCaptchaProvider(); const config = CAPTCHA_PROVIDERS[provider]; if (config) { try { localStorage.setItem(config.keyStorage, k); logStatus(`✓ ${config.name} key saved`, 'g'); } catch(e) { logStatus(`❌ Save error`, 'r'); } } });
    document.getElementById('ivac-btn-captcha-reset')?.addEventListener('click', () => { const provider = getSelectedCaptchaProvider(); const config = CAPTCHA_PROVIDERS[provider]; if (config) localStorage.removeItem(config.keyStorage); const inp = document.getElementById('ivac-captcha-api-input'); if (inp) inp.value = ''; logStatus(`⚠ ${config?.name || 'Captcha'} key cleared`, 'y'); });
    (() => { const provider = getSelectedCaptchaProvider(); const sel = document.getElementById('ivac-captcha-provider-select'); if (sel) sel.value = provider; const config = CAPTCHA_PROVIDERS[provider]; if (config) { const savedKey = localStorage.getItem(config.keyStorage) || ''; const inp = document.getElementById('ivac-captcha-api-input'); if (savedKey && inp) inp.value = savedKey; } })();
    document.getElementById('ivac-captcha-provider-select')?.addEventListener('change', (e) => { const provider = e.target.value; setSelectedCaptchaProvider(provider); const config = CAPTCHA_PROVIDERS[provider]; if (config) { const savedKey = localStorage.getItem(config.keyStorage) || ''; const inp = document.getElementById('ivac-captcha-api-input'); if (inp) inp.value = savedKey; logStatus(`🔄 Captcha provider: ${config.name}`, 'g'); } renderTokenQueue(); });
    ...
    function silentGMRequest(url, body, timeoutMs) {
        return new Promise((resolve, reject) => {
            ...
            gmApi({ method: 'POST', url: url, headers: { 'Content-Type': 'application/json' }, data: JSON.stringify(body), timeout: timeoutMs || 20000,
    ...
    let _captchaSolvingActive = false; let _lastSilentSolveTime = 0; const SILENT_SOLVE_COOLDOWN = 500;
    async function getCaptchaTokenSmart() {
        const useApi = document.getElementById('captcha-toggle')?.classList.contains('on');
        if (!useApi) {
            showManualCaptcha(); // manual mode → render & show the widget now that a token is needed
            for (let i = 0; i < 60; i++) {
        ...
        if (_captchaSolvingActive) { let waited = 0; while (_captchaSolvingActive && waited < 30000) { await new Promise(r => setTimeout(r, 500)); waited += 500; const queued = tokenQueue.find(t => t.source === providerTag); if (queued) return queued.token; } }
    ...
    async function stepSignin(signal) {
        ...
        let signinTimeoutId = null; let signinTimeoutFired = false;
        if (signinTimeoutOn) { signinTimeoutId = setTimeout(() => { logStatus('⏱ Signin timeout (20s) — forcing retry', 'y'); signinTimeoutFired = true; localAc.abort(); }, 20000); }
        ...
        } catch (err) { if (signinTimeoutId) { clearTimeout(signinTimeoutId); signinTimeoutId = null; } if (err.name === 'AbortError') { netLogUpdate(logId, { state: 'cancel', status: '⊘' }); return { win: false, cancelled: signinTimeoutFired ? false : true }; } netLogUpdate(logId, { state: 'fail', status: 'err', note: err.message }); return { win: false, cancelled: false }; } finally { try { signal?.removeEventListener('abort', onParentAbort); } catch(e) {} try { unregisterTokenInFlight(captchaToken, localAc); } catch(e) {} }
    }
    ...
    async function stepVerify(signal) {
        ...
        try {
            const r = await H2.fetchH2(API_VERIFY, { method: 'POST', signal, headers: { 'accept': 'application/json', 'authorization': `Bearer ${sessionState.accessToken}`, 'content-type': 'application/json', 'x-device-id': getDeviceId() }, referrer: API_REFERRER, body: JSON.stringify({ requestId: sessionState.requestId, phone: sessionState.phone, code: otp, otpChannel: 'PHONE' }) });
    ...
    async function stepReserve(signal) {
        ...
        const encryptedCaptchaToken = encManager.encryptToken(captchaToken, 'Reserve');
        const localAc = new AbortController(); const onParentAbort = () => { try { localAc.abort(); } catch(e) {} }; signal?.addEventListener('abort', onParentAbort); registerTokenInFlight(captchaToken, localAc);
    ...
    async function stepBook(signal) {
        try {
            ...
            if (savedApptId && savedApptId.length >= 10) {
                sessionState.appointmentId = savedApptId; sessionState.bookedAt = Date.now(); persistSession();
                logStatus(`⏭ Book skipped — saved appointmentId: ${savedApptId}`, 'g'); try { beepBook(); } catch(e) {}
                return { win: true, skipped: true, fromProfile: true };
            }
        } catch(e) {}
        if (!_forceStep) { if (isStepAlreadyDone('book')) { logStatus(`⏭ Book already done — using session appointmentId`, 'g'); return { win: true, skipped: true }; } }
        if (!sessionState.accessToken) { if (_forceStep) logStatus('⚠ No session', 'y'); else { logStatus('❌ No session', 'r'); return { win: false }; } }
        if (raceCoord.hasWon('book')) { logStatus(`⏭ Book race already won — bailing`, 'y'); return { win: false, cancelled: true }; }
        const logId = netLogAdd({ method: 'GET', url: API_BOOK, tag: 'book', state: 'pending' });
        try {
            const r = await H2.fetchH2(API_BOOK, { method: 'GET', signal, headers: { 'accept': 'application/json', 'authorization': `Bearer ${sessionState.accessToken}`, 'content-type': 'application/json' }, referrer: API_REFERRER, body: null });
            let body = null; try { body = await r.json(); } catch(e) { body = null; }
            ...
            if (ok && body.data) {
                ...
                sessionState.abcSlot = body.data.appointmentSlot || null; sessionState.ivacCenter = body.data.ivacCenter || null;
                sessionState.mission = body.data.mission || null; sessionState.numberOfApplicants = body.data.numberOfApplicants || null;
                sessionState.totalAmount = body.data.totalAmount || null; sessionState.fileUploadStatus = body.data.fileUploadStatus || null;
                sessionState.visaCodes = body.data.visaCodes || null; sessionState.bookedAt = Date.now(); persistSession();
                try { if (body.data.appointmentId && profiles[activeProfileName]) { profiles[activeProfileName].appointmentId = body.data.appointmentId; persistProfiles(); if (pmAppointmentId) pmAppointmentId.value = body.data.appointmentId; } } catch(e) {}
    ...
    document.getElementById('ivac-btn-fetch-send')?.addEventListener('click', async () => {
        ...
        try {
            ...
            document.getElementById('ivac-fetch-input').value = JSON.stringify({ url: config.url, method: method, headers: headers, body: body ? (typeof config.body === 'object' ? config.body : JSON.parse(body)) : null, response: { status: r.status, statusText: r.statusText, body: pretty.slice(0, 2000) + (pretty.length > 2000 ? '...' : '') } }, null, 2);
    ...
    (function manualPanelModule() {
        ...
        function mpFindButtonByText(matchers) { for (const b of mpVisibleButtons()) { const txt = (b.textContent || b.innerText || '').trim().toLowerCase(); if (matchers.some(m => txt === m || txt.includes(m))) return b; } return null; }
        ...
        function mpGetPhone() { const fromSession = sessionState.phone; const fromProfile = (profiles[activeProfileName]?.phone1 || '').trim(); const pageInp = document.querySelector('input[name="phone"], input[type="tel"], input[placeholder*="01"]'); const fromPage = pageInp && pageInp.value ? pageInp.value.trim() : ''; return fromSession || fromProfile || fromPage || (document.getElementById('login-phone')?.value || '').trim(); }
        function mpFillOtpToPage(otp) {
            ...
            if (boxes.length >= 6) { for (let i = 0; i < 6; i++) { mpSetNativeValue(boxes[i], otp[i]); if (i < 5 && boxes[i+1]) setTimeout(() => boxes[i+1].focus(), 25 * i); } setTimeout(() => { try { boxes[5].blur(); boxes[5].dispatchEvent(new Event('blur', { bubbles: true })); } catch(e) {} }, 250); setTimeout(() => { const vb = mpFindVerifyBtn(); mpEnableBtn(vb); }, 300); return true; }
        ...
        document.getElementById('mp-btn-number-password')?.addEventListener('click', () => {
            ...
            const phone = (prof.phone1 || '').trim(); const pass = prof.mobilePass || '';
        ...
        document.getElementById('mp-get-otp-btn')?.addEventListener('click', () => {
            ...
            startSmsFetcher(phone, async (otp) => { const mi = document.getElementById('mp-otp-input'); if (mi) { mi.value = otp; mi.style.background = '#1a3a1a'; setTimeout(() => mi.style.background = '#333', 1200); } const li = document.getElementById('login-otp'); if (li) li.value = otp; mpFillOtpToPage(otp); logStatus(`📩 Manual OTP: ${otp}`, 'g'); return undefined; }, false, true);
        });
        ...
        function mpAutoCloseNotices() { try { document.querySelectorAll('button[aria-label="Close notice"], button[aria-label="Close popup"]').forEach(b => { if (b.offsetParent !== null) { b.click(); } }); } catch(e) {} }
        try { const mo = new MutationObserver(() => mpAutoCloseNotices()); mo.observe(document.body, { childList: true, subtree: true }); setInterval(mpAutoCloseNotices, 1500); mpAutoCloseNotices(); } catch(e) {}
