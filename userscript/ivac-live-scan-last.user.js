...
<div class="fr" style="align-items:center"> <label style="flex:none;display:flex;align-items:center;gap:2px;font-size:.55rem;color:#7777aa;font-weight:700;cursor:pointer" title="✓ = Reserve sends RAW token (skip encryption). Unchecked = encrypted (default)."><input type="checkbox" id="chk-reserve-raw" style="width:12px;height:12px;accent-color:#f59e0b;cursor:pointer">raw</label> <button class="b1 bh" style="flex:1" id="brs">Reserve</button> <button class="b14 bh" style="flex:1" id="bbk">Book</button> <button class="b6 bh" style="flex:1" id="bin">Initiate</button>
</div>
<div class="fr" style="gap:6px;align-items:center"><input type="text" id="ivac-reserve-slot-id" placeholder="Reserve Slot ID (ccd3dd63-… — center fixed)" autocomplete="off" spellcheck="false" style="flex:1;min-width:0" title="Center-fixed slot UUID from the real reserve-slot URL. Paste once; saved."><label style="font-size:.62rem;color:#7777aa;font-weight:700;white-space:nowrap">🔔 Popup</label><div class="tg" id="popup-toggle" title="Popup: ON=show milestone popups, OFF=disable"><div class="tg-dot"></div></div></div>
...
<div class="ck-row"> <span class="ck-time" id="tk">12:13:17 AM</span> <span class="ck-sep"></span> <span class="ck-date" id="dk">08/05/2026</span> <button class="b8" id="blo" style="margin-right:-6px;padding:3px 15px">Logout</button>
</div>
</div>
</div>
</div>
...
// Signin/Reserve raw-token checkboxes (checked = raw, unchecked = encrypted) — persisted
(function initRawTokenChecks() {
    [['chk-signin-raw', 'rj_chk_signin_raw'], ['chk-reserve-raw', 'rj_chk_reserve_raw']].forEach(([id, key]) => {
