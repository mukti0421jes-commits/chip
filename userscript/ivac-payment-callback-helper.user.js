...
(function () {
  ...
  var BRIDGE_KEY = 'rj_pay_callback';   // cross-page handoff: gateway -> ivacbd

  // ---------------- callback URL cinha (shared) ----------------
  ...
  // ======================================================================
  // PART B — IVACBD SIDE: detect (bridge + own traffic) + retry loop + UI
  // ======================================================================
  ...
  // PerformanceObserver — page e ghota SOB network request er URL dhore
  try {
  ...
  // poll kore purono entry + bridge scan (buffered miss korle)
  ...
  // CROSS-PAGE BRIDGE: gateway theke stash kora callback URL pore nao
  function readBridge() {
    try {
      var u = localStorage.getItem(BRIDGE_KEY);
      if (u && isCallbackUrl(u)) { onDetect(u); }
    } catch (e) {}
  }
  // onno tab/page (gateway) bridge lekhle sathe sathe dhoro
  window.addEventListener('storage', function (ev) { if (ev && ev.key === BRIDGE_KEY && ev.newValue && isCallbackUrl(ev.newValue)) onDetect(ev.newValue); });
  readBridge();
  ...
  function startLoop() {
    ...
    timer = setInterval(fireOnce, DELAY_MS);
  }
  ...
  function succeed(reason) {
    ...
    try { localStorage.removeItem(BRIDGE_KEY); } catch (e) {}   // bridge clear kore rakho
