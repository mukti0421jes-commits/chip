...
(function() {
    ...
    async function performSignin(phone, password, captchaToken) {
        ...
        const response = await H2.fetchH2(API_SIGNIN_V2, {
            method: "POST",
            headers: { "accept": "application/json, text/plain, */*", "cache-control": "no-cache, no-store, must-revalidate", "content-type": "application/json", "pragma": "no-cache" },
  ...
  <div class="bd">
    ...
    <div class="tp a" id="pl">
      <div class="sc">
        ...
        <div class="fr sa-row">
          <input type="number" id="rt-signin"   value="2"  min="0" max="999" title="Signin retry delay (seconds)">
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">`;
    ...
    // ==================== RESERVE: fixed slot id + date picker ====================
    // Reserve endpoint is /slots/{slotId}/reserve-slot with body {c, appointmentDate}. slotId is a
    // fixed constant (same across centers); the Slot ID box can override it. Date comes from the
    // dropdown (auto-synced from the time-slot page or get-booking-config).
    ...
    function saveReserveSlotId(v) { v = _cleanUuid(v); if (!v) return; try { localStorage.setItem(RESERVE_SLOT_ID_KEY, v); } catch(e) {} }
    ...
    // Scrape dates the site shows on /appointment/time-slot (DD-MM-YYYY dash leaves), skip our own UI.
    function scrapePageDates() {
        const set = new Set();
        const onlyDash = (t) => { const m = /^(\d{2})-(\d{2})-(\d{4})$/.exec(t); if (m) return `${m[3]}-${m[2]}-${m[1]}`; const m2 = /^(\d{4})-(\d{2})-(\d{2})$/.exec(t); return m2 ? `${m2[1]}-${m2[2]}-${m2[3]}` : ''; };
        try {
            document.querySelectorAll('button, span, li, div, option, p').forEach(el => {
                if (el.children && el.children.length) return;
                if (el.closest('#p, #rj-netlog, #ivac-manual-dom-modal, [id^="ivac-"], [id^="rj-"], [id^="mp-"]')) return;
    ...
    // Auto-sync: mirror the page's date list; when off the page, pull from get-booking-config (throttled).
    (function initReserveDateAutoSync() {
        let last = ''; let lastApiTry = 0;
        const tick = async () => {
            try {
                const sel = document.getElementById('ivac-reserve-date'); if (!sel) return;
                const dates = scrapePageDates();
                if (dates.length) { const sig = dates.join(','); if (sig !== last) { last = sig; populateReserveDates(dates); } return; }
                const hasDates = sel.options.length > 1;
                if (!hasDates && sessionState.accessToken && (Date.now() - lastApiTry > 20000)) { lastApiTry = Date.now(); try { await loadReserveDates(); } catch (e) {} }
            } catch (e) {}
        };
    ...
    async function stepReserve(signal) {
        ...
        try {
            const r = await H2.fetchH2(RESERVE_URL, { method: 'POST', signal: localAc.signal, headers: { 'accept': 'application/json, text/plain, */*', 'authorization': `Bearer ${sessionState.accessToken}`, 'cache-control': 'no-cache, no-store, must-revalidate', 'content-type': 'application/json', 'pragma': 'no-cache' }, referrer: API_REFERRER, credentials: 'include', body: JSON.stringify({ c: encryptedCaptchaToken, appointmentDate }) });
    ...
    async function stepBook(signal) {
        ...
        try {
            const r = await H2.fetchH2(API_BOOK, { method: 'GET', signal, headers: { 'accept': 'application/json, text/plain, */*', 'authorization': `Bearer ${sessionState.accessToken}`, 'cache-control': 'no-cache, no-store, must-revalidate', 'pragma': 'no-cache' }, referrer: API_REFERRER, body: null });
            let body = null; try { body = await r.json(); } catch(e) { body = null; }
