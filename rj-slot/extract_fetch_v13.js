#!/usr/bin/env node
// extract_fetch.js — IVAC bundle extractor v13
// ─────────────────────────────────────────────────────────────────────────────
// v13 CHANGES vs v12:
//   • NEW Strategy K — VM-based property evaluation. The most robust strategy
//     yet: instead of parsing alias function bodies with regex, we eval each
//     property value expression in a real JS sandbox where:
//       1. The rotated array (hU) is live
//       2. All decoder functions (mU/WU/CU/etc.) are provided as closures
//       3. All alias function definitions from genBody are run as real JS
//     This makes the extractor pattern-independent: arrow functions,
//     ternary-wrapped aliases, multiplication expressions, and other
//     obfuscator variations all work because the alias code runs as JS,
//     not parsed. Strategy K is the universal fallback when A–J fail.
//
// v12 CHANGES vs v11:
//   • BUG FIX — SLOT_UUID rescue for typo'd UUIDs in /slots/<id>/reserve-slot
//     literals. Some bundles (e.g. mtv1rx02-e9bAiBuo.js) ship a slot UUID with
//     a non-hex trailing char in the last 12-char segment, e.g.
//       /slots/139fd4d2-27c9-4758-a623-368583e830bs/reserve-slot
//                                            ↑ 's' is non-hex
//     Strict UUID_RE /[0-9a-f]{8}-[0-9a-f]{4}-...-[0-9a-f]{12}/ won't match
//     this, so SLOT_UUID fell through to "SLOT_ID_NOT_FOUND" and fetch-api.js
//     lost the slot ID entirely. v12 adds Strategy I (lenient slot UUID
//     rescue) that scans /slots/<36-char-with-possible-non-hex-trailer>/
//     patterns explicitly and tolerates one non-hex char in the final slot.
//   • NEW Strategy J — direct mU()/WU() call scan in generator body for
//     bundles whose local alias functions map directly to mU/WU (not the
//     *V-suffixed variants that parseGenAliasesExtended handles). Real
//     example from mtv1rx02-e9bAiBuo.js:
...
//       UCONv:"payme"+o(275,"g!Tz")+u(-449,"kJh5")+...+mU(250,"$VRW")
//             +t(-68,"1mnJ")+r(-535)+WU(180)+t(-20,"[aFw")+"e"
//     → "/payment/23228961-2326-3s28-861f-465bb28337a3/dg-epay/initiate"
//     (note: UUID also has a non-hex 's' in segment 3 — same bundle typo
//     pattern as the slot UUID).
//   • Verification vc("SLOT_UUID") regex updated to accept the same lenient
//     shape so the post-extraction check passes.
//
// v11 CHANGES vs v10:
...
console.log("🔍 IVAC Fetch Extractor v13");
  ...
  if (!dgPath) {
    ...
    try {
      ...
      if (aliasDefs.length === 0) {
        ...
        try {
          vm.runInContext(aliasDefs.join('\n'), sandbox);
        } catch (e) {
        ...
        while ((pkm = propReK.exec(genBody)) !== null) {
          ...
          let result;
          try {
            result = vm.runInContext('(' + val + ')', sandbox, { timeout: 1000 });
          } catch (e) { continue; }
...
// v12: SLOT_UUID check now accepts non-hex chars anywhere in the UUID
// segments because some bundles ship typo'd UUIDs (e.g.
// 139fd4d2-27c9-4758-a623-368583e830bs with stray 's' at end of last seg,
// or 23228961-2326-3s28-861f-465bb28337a3 with 's' in segment 3).
// We still require the full 8-4-4-4-12 shape.
vc("SLOT_UUID",     SLOT_UUID,      v=>!!v&&v!=="SLOT_ID_NOT_FOUND"&&/^[0-9a-z]{8}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{12}$/i.test(v));
vc("DGEPAY_UUID",   DGEPAY_UUID,    v=>!!v&&v!=="DGEPAY_ID_NOT_FOUND"&&/^[0-9a-z]{8}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{12}$/i.test(v));
if (DECODED_DGEPAY_PATH) {
...
const GENERATED=`// fetch-api.js — AUTO-GENERATED by extract_fetch.js v13
