...
async function _auAppointmentUntilSuccess() {
    ...
    while (_auGateOn()) {
        ...
        for (let w = 0; w < 24 && !sessionState.appointmentCreated && !sessionState.appointmentId; w++) await _auSleep(300);
        if (sessionState.appointmentCreated || sessionState.appointmentId) { logStatus('✅ Appointment created — starting uploads', 'g'); return true; }
...
refreshProxyPicker(); refreshProxyStatusLine(); updateActiveProxyGlobal();
    ...
    document.getElementById('ivac-btn-appointment')?.addEventListener('click', async function() {
        ...
        try {
            ...
            let body = null; try { body = await r.json(); } catch(e) { body = null; }
            // Appointment success = HTTP 2xx. It returns NO appointmentId (that comes later from get-booking-config),
            // so an in-body statusCode must NOT gate this flag — otherwise the auto-upload loop never sees success
            // and re-clicks Appointment forever (the "appointment loops, upload never starts" bug).
            const ok = !!(r && r.status >= 200 && r.status < 300);
            netLogUpdate(logId, { status: r.status, state: ok ? 'ok' : 'fail', note: ok ? `appointmentId=${body?.data?.appointmentId?.slice(0,8) || 'ok'}` : (body?.message || `HTTP ${r.status}`) });
            if (ok) { const d = (body && body.data) || {};
