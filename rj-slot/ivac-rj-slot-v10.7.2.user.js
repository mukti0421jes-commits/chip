...
async function _auAppointmentUntilSuccess() {
    ...
    while (_auGateOn()) {
        ...
        // success signal = appointmentCreated (set on HTTP 2xx by the handler). The /appointment POST does
        // NOT return an appointmentId — that arrives later from get-booking-config (Book) — so we do NOT
        // wait on appointmentId here (a stale/profile id is already handled by the skip check above).
