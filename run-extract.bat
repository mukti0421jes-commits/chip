...
REM  extract_ciphers.js on it. No need to edit anything when the
REM  bundle's name changes.
REM
REM  Keep this file in the autocheck folder (next to extract_ciphers.js).
REM ====================================================================
...
set "BUNDLE="
for /f "delims=" %%F in ('dir /b /a-d /o-s *.js 2^>nul ^| findstr /v /i /b /c:"extract_ciphers.js" /c:"cipher.js" /c:"cipher-server.js" /c:"start-cipher-hidden.js"') do (
...
echo.
echo [DONE] cipher.js has been regenerated. Now just reload the IVAC page.
pause
