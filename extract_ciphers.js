...
function resolveExpr(expr,pos){
  ...
  const arr=[...arrset];if(arr.length===0)return null;
  const arrays={};for(const an of arr){const b=getArr(an);if(!b)return null;arrays[an]=b;}
  // helper: build a decoder code body. `rots` maps arrayName->rotation. `liveOnly`=array whose
  // fns are real; if set, other arrays' base fns return "" (used to pin each rotation independently).
  function build(rots,liveOnly){
    let code="";
    for(const[n,d]of Object.entries(need.base)){
      if(liveOnly&&d.arrfn!==liveOnly){code+=`const ${n}=()=>"";\n`;continue;}
      code+=`const ${n}=(e,t)=>{const r=__A[${JSON.stringify(d.arrfn)}][e-${d.offset}];return r===undefined?null:(${d.rc4?"__rc4(r,t)":"__b64(r)"});};\n`;
    }
    for(const[n,w]of Object.entries(need.wrap))code+=`function ${n}(e,t){return ${w.base}(${w.inner})}\n`;
    const __A={};for(const an of arr){const b=arrays[an];const rot=rots[an]||0;__A[an]=b.slice(rot).concat(b.slice(0,rot));}
    return {code,__A};
  }
  const clean=v=>typeof v==="string"&&/^[\x20-\x7e]+$/.test(v)&&v.length>=1;
  // 1) pin candidate rotations for each array independently
  const cand={};
  for(const an of arr){
    cand[an]=[];const len=arrays[an].length;
    for(let rot=0;rot<len;rot++){
      const {code,__A}=build({[an]:rot},an);
      try{const v=new Function("__A","__rc4","__b64",code+"return ("+expr+")")(__A,rc4,b64);
        if(typeof v==="string"&&v.length>0&&/^[\x20-\x7e]*$/.test(v))cand[an].push(rot);}catch(e){}
    }
    if(cand[an].length===0)cand[an]=Array.from({length:len},(_,i)=>i); // fall back to all
  }
  // 2) try combinations (usually 1 each) with all arrays live, require full clean string
  const names=arr,rots={};
  function tryCombo(i){
    if(i===names.length){
      const {code,__A}=build(rots,null);
      try{const v=new Function("__A","__rc4","__b64",code+"return ("+expr+")")(__A,rc4,b64);
        if(clean(v)&&v.length>=3)return v;}catch(e){}
      return null;
    }
    const an=names[i];for(const rot of cand[an]){rots[an]=rot;const r=tryCombo(i+1);if(r)return r;}
    return null;
  }
  return tryCombo(0);
}
