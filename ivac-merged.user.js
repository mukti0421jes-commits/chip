...
// @version      10.0.0-merged
// @description  RJ SLOT v7.5 engine + Manual Panel clone. Manual buttons reuse RJ engine (signin/verify/reserve/book/initiate, token queue, SMS fetcher, profiles, popups). Per-button ON=API fetch / OFF=DOM click on manual page. Both UIs minimized on load.
// @author       RJ SLOT
...
(function() {
    ...
    const encManager = {
        ...
        saveUnifiedScript(scriptText) {
            try { localStorage.setItem(ENC_UNIFIED_SCRIPT_KEY, scriptText || ''); } catch(e) {}
        },

        getSavedUnifiedScript() {
            try { return localStorage.getItem(ENC_UNIFIED_SCRIPT_KEY) || ''; } catch(e) { return ''; }
        },

        saveEnabledCheck(checked) {
            try { localStorage.setItem(ENC_UNIFIED_CHECK_KEY, checked ? '1' : '0'); } catch(e) {}
        },

        getSavedEnabledCheck() {
            try { return localStorage.getItem(ENC_UNIFIED_CHECK_KEY) === '1'; } catch(e) { return false; }
        },
    ...
    <div class="tp a" id="pl">
      <div class="sc">
        ...
        <div class="fr sa-row">
          ...
          <input type="number" id="rt-reserve"  value="1" min="0" max="999" title="Reserve retry delay (seconds)">
          ...
          <button class="b8 toggle-btn sa-btn" id="btn-single" title="Single ON: retry on failure. OFF: no retry, fail = stop">Single</button>
          <input type="number" id="rt-auto-loops" value="0" min="0" max="999" title="Reserved (currently unused)">
          <button class="b8 toggle-btn sa-btn" id="btn-auto" title="Auto ON: after a step wins, automatically continues to next step. Manual button starts the flow.">Auto</button>
        </div>
        <div class="fr-phone">
...
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">`;
    ...
    function makeDraggable(dragHandle, targetElement) {
        ...
        document.addEventListener('mouseup', function() {
            if (isDragging) { isDragging = false; targetElement.style.transition = ''; }
        });
    }
    ...
    document.getElementById('ivac-btn-submit-info')?.addEventListener('click', async function() {
        ...
        if (!givenName) missing.push('Given Name');
        if (!password) missing.push('Password');
        if (!nid && !passport) missing.push('NID/Passport');
        if (!dobRaw) missing.push('DOB');
        ...
        suMsg('ivac-msg-submit', '⏳ Creating account...', 'y');
        logStatus(`📝 Creating account for ${email}...`, 'y');
        const logId = netLogAdd({ method: 'POST', url: API_SIGNUP, tag: 'signup', state: 'pending', note: 'account-registration' });
        try {
            let captchaToken;
            try { captchaToken = await getCaptchaTokenSmart(); }
            catch(e) { suMsg('ivac-msg-submit', `❌ Captcha: ${e.message}`, 'r'); netLogUpdate(logId, { state: 'fail', note: 'captcha failed' }); flashButton(btn, '✗', 'r'); return; }
            ...
            if (ok) {
                ...
                netLogUpdate(logId, { status: response.status, state: 'fail', note: msg });
            }
    ...
    async function stepBook(signal) {
        if (!_forceStep) {
            try { const savedApptId = profiles[activeProfileName]?.appointmentId?.trim().replace(/^["']|["']$/g, ''); if (savedApptId && savedApptId.length >= 10) { sessionState.appointmentId = savedApptId; sessionState.bookedAt = Date.now(); persistSession(); logStatus(`⏭ Book skipped — saved appointmentId`, 'g'); try { beepBook(); } catch(e) {} return { win: true, skipped: true, fromProfile: true }; } } catch(e) {}
            if (isStepAlreadyDone('book')) { logStatus(`⏭ Book already done`, 'g'); return { win: true, skipped: true }; }
        }
        if (!sessionState.accessToken) { if (_forceStep) logStatus('⚠ No session', 'y'); else { logStatus('❌ No session', 'r'); return { win: false }; } }
        ...
        try {
            ...
            let body = null; try { body = await r.json(); } catch(e) {} const ok = r.ok && body && (body.successFlag === true || body.message === 'Success' || body.statusCode === 200);
            netLogUpdate(logId, { status: r.status, state: ok ? 'ok' : 'fail', note: ok ? `appointmentId ${body.data?.appointmentId?.slice(0,8)||'?'}` : (body?.message || `HTTP ${r.status}`) });
            if (ok && body.data) {
                raceCoord.declareWin('book', { win: true, data: body });
                sessionState.appointmentId = body.data.appointmentId || sessionState.appointmentId; sessionState.abcDate = body.data.appointmentDate || sessionState.abcDate; sessionState.abcSlot = body.data.appointmentSlot || null; sessionState.ivacCenter = body.data.ivacCenter || null; sessionState.mission = body.data.mission || null; sessionState.numberOfApplicants = body.data.numberOfApplicants || null; sessionState.totalAmount = body.data.totalAmount || null; sessionState.fileUploadStatus = body.data.fileUploadStatus || null; sessionState.visaCodes = body.data.visaCodes || null; sessionState.bookedAt = Date.now(); persistSession();
                ...
                logStatus(`✅ Booked: ${body.data.ivacCenter||''} ${body.data.appointmentSlot||''} • ৳${body.data.totalAmount||'?'}`, 'g'); try { beepBook(); } catch(e) {}
    ...
    //  - Unique mp- IDs (avoid collision with RJ Upload tab)
    //  - Per-button toggle ON = API fetch via RJ engine; OFF = DOM click
    //  - Single/Auto mirror RJ engine toggles; retry delay from mp sec inputs
    // ==================================================================
    (function manualPanelModule() {
        const mpStop = { value: false };
        function manualStopAllImpl() { mpStop.value = true; }
        // expose to RJ Stop All
        ...
        // ---------- draggable (reuse RJ makeDraggable) ----------
        try { makeDraggable(document.getElementById('mp-header'), modal); } catch(e) {}

        // ---------- profile dropdown (synced with RJ profiles) ----------
        ...
        // ---------- shared Single/Auto with RJ engine ----------
        ...
        async function mpDomClickLoop(label, finder, secInputId, postEnable) {
            ...
            while (!mpStop.value) {
                ...
                if (!isSingleOn()) break;        // Single OFF → single hit
                await new Promise(r => setTimeout(r, sec * 1000));
            }
        }

        // ---------- API mode via RJ engine (mirror sec → rt-*) ----------
        ...
        document.getElementById('mp-btn-number-password')?.addEventListener('click', () => {
            ...
            // also fill RJ login fields
            const lp = document.getElementById('login-phone'); const lpass = document.getElementById('login-password');
            if (lp) lp.value = phone; if (lpass) lpass.value = pass;
            // fill the manual PAGE inputs (react-safe)
            ...
            // keep Sign In Now button enabled
        ...
        document.getElementById('mp-verify-btn')?.addEventListener('click', async () => {
            // push manual OTP (if typed) into RJ + page first
        ...
        // ---------- Auto-close notice/popup on the page ----------
        ...
        // both UIs start minimized (modal hidden + fab shown) — already set via classes
